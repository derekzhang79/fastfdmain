    ...
    ui(new Ui::material_purchase_apply)
{
    ui->setupUi(this);
    //
    ui->comboBox_CH_STATE->addItem(tr("未申请"), "0");
    ui->comboBox_CH_STATE->addItem(tr("全部"), "");
    ui->comboBox_CH_STATE->addItem(tr("已申请"), "1");
    ui->comboBox_CH_STATE->addItem(tr("已审核"), "2");
    ...
    tablemodel_master->save_set_header(tablemodel_master->fieldIndex("APPLY_PEOPLE"), tr("申请人"));
...
void material_purchase_apply::toaudit()
{
    ...
    if("0" != CH_STATE) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("当前状态")+":"+
...
void material_purchase_apply::todel()
{
    ...
    if(row >= 0) {
        ...
        QString CH_STATE = tablemodel_master->model_data(row, "CH_STATE").toString();
        if(CH_STATE == "1") {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("已申请")+"," +tr("操作失败"));
            return;
        }
        if(CH_STATE == "1") {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("已审核")+"," +tr("操作失败"));
            return;
        }
...
//    headInfo.insert("machineCode","");
    ...
    QString http_key="http://" + n_func::f_get_sysparm("cloud_ip")+
            "/CateSystem/arm/material-purchase-apply/getByBillNo-rest";


    cJSONHttpData json_yun_readcard =  w_sys_manage_cloudsync::get_yun_httpget(this, http_key, headInfo,tr("采购申请"), false, true, 10*1000);
...
void material_purchase_apply::billnApp(const QString &BILL_NO)
{
    ...
    cJSON_AddItemToObject(root.json, "materialPurchaseApply", materialPurchaseApply);
    cJSON_AddItemToObject(root.json, "materialPurchaseApplyDetail", materialPurchaseApplyDetail);
    ...
    query.execSelect(QString("SELECT * from material_purchase_apply where BILL_NO ='%1'  ")
                     .arg(BILL_NO));
    query.next();
    cJSON_help::_cJSON_AddStringToObject(materialPurchaseApply, "billNo", query.recordValue("BILL_NO"));
    cJSON_help::_cJSON_AddStringToObject(materialPurchaseApply, "applyPeople", query.recordValue("APPLY_PEOPLE"));
    cJSON_help::_cJSON_AddStringToObject(materialPurchaseApply, "remark", query.recordValue("REMARK"));
    cJSON_help::_cJSON_AddStringToObject(materialPurchaseApply, "title", query.recordValue("TITLE"));
    cJSON_help::_cJSON_AddStringToObject(materialPurchaseApply, "orgCode", orgCode);
    ////////////////
    ...
    while(query.next()) {
        ...
        cJSON_help::_cJSON_AddStringToObject(materialPurchaseApplyDetail0, "materialCode", query.recordValue("MATERIAL_CODE"));
        cJSON_help::_cJSON_AddStringToObject(materialPurchaseApplyDetail0, "materialName", query.recordValue("MATERIAL_NAME"));
        cJSON_help::_cJSON_AddStringToObject(materialPurchaseApplyDetail0, "format", query.recordValue("FORMAT"));
        cJSON_help::_cJSON_AddStringToObject(materialPurchaseApplyDetail0, "unitCode", query.recordValue("UNIT_CODE"));
        cJSON_help::_cJSON_AddStringToObject(materialPurchaseApplyDetail0, "unitName", query.recordValue("UNIT_NAME"));
        cJSON_help::_cJSON_AddStringToObject(materialPurchaseApplyDetail0, "applyNum", query.recordValue("APPLY_NUM"));
        cJSON_help::_cJSON_AddStringToObject(materialPurchaseApplyDetail0, "orgCode", orgCode);
    }
...
//    headInfo.insert("machineCode","");
    ...
    headInfo.insert("requestType","CY");
    QString http_key="http://" + n_func::f_get_sysparm("cloud_ip")+
            "/CateSystem/arm/material-purchase-apply/add-rest";


    cJSONHttpData json_yun_readcard =  w_sys_manage_cloudsync::get_yun_httppos(this, http_key, param, headInfo,tr("采购申请"));
