    ...
    ui(new Ui::material_purchase_apply_new)
{
    ...
    tablemodel_detail->save_set_header(tablemodel_detail->fieldIndex("APPLY_NUM"), tr("申请数量"), true);
...
bool material_purchase_apply_new::saveData()
{
    ...
    if(tablemodel_detail->rowCount() == 0) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("表单内容为空"));
        return false;
    }
    if(ui->APPLY_PEOPLE->curusrdata().toString().isEmpty()) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("申请人不能为空"));
        return false;
    }
    for(int row = 0; row < tablemodel_detail->rowCount(); row++) {
        ...
        if(tablemodel_detail->model_data(row, "MATERIAL_CODE").toString().isEmpty()) {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("行数")+QString::number(row+1) + "," +tr("表单编码不能为空"));
            return false;
        }
        if(tablemodel_detail->model_data(row, "APPLY_NUM").toDouble() <= 0) {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("行数")+QString::number(row+1) + "," + tr("表单申请数量必须大于0"));
            return false;
        }
    }
    //BILL_NO
