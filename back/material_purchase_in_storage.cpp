...
//    headInfo.insert("machineCode","");
    ...
    QString http_key="http://" + n_func::f_get_sysparm("cloud_ip")+
            "/CateSystem/arm/material-stote-in-storage/getDistribution-rest";
...
//    headInfo.insert("machineCode","");
    ...
    QString http_key="http://" + n_func::f_get_sysparm("cloud_ip")+
            "/CateSystem/arm/material-stote-in-storage/submitByBillNo-rest";
...
bool material_purchase_in_storage::local_inventory_in_storage(QWidget *parent, const QString &billNo, const QString &yun_table_master, const QString &yun_table_detail, const QString &key)
{
    ...
    if(!query.execInsert("cey_st_sheet_master",qrtVariantPairList()
                         ...
                         << qrtVariantPair("ch_operation", backheader::VIPFINDVALUE(w_i_inventory::get_MODEL_INVENTORY_SHEET_TYPE(), key))
    ...
    while(query.next()) {
        ...
        if(!query2.next()) {
            lds_messagebox::warning(parent, MESSAGE_TITLE_VOID ,tr("入库商品") + tr("本地不存在")+","+tr("入库商品编码")+":"+ch_dishno);
            goto failure;
        }
        if("Y" != query2.recordValue("item_flag").toString()) {
            lds_messagebox::warning(parent, MESSAGE_TITLE_VOID ,tr("入库商品")+tr("不支持库存管理")+","+tr("入库商品编码")+":"+ch_dishno);
            goto failure;
        }
        //显示入库后的库存
    ...
    //
    {
        ...
        if(!msg.isEmpty()) {
            if(1 == lds_messagebox::information(parent, MESSAGE_TITLE_VOID, tr("库存数量") + "\n"+ msg, tr("确认"), tr("取消"))) {
                goto failure;
            }
        }
    }
