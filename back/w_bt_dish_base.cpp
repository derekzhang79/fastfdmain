    ...
    ui(new Ui_w_bt_dish_Dialog)
{
    ...
    tablemodel->save_set_header("ch_groupno",tr("组号"));
    tablemodel->save_set_header("vch_dishname", tr("菜品名称"));
    tablemodel->save_set_header("ch_unitno", tr("菜品单位"));
    tablemodel->save_set_header("num_price", tr("菜品价格"));
    tablemodel->save_set_header("num_m_price", tr("会员特价1"));

    tablemodel->save_set_header("ch_stopflag", tr("停用"));
    tablemodel->save_set_header("ch_material_flag",tr("是否原料"));
    tablemodel->save_set_header("ch_weight",tr("可称重"));
    tablemodel->save_set_header("ch_suitflag",tr("套菜"));
    tablemodel->save_set_header("ch_curprice", tr("时价菜"));
    ...
    tablemodel->save_set_header("ch_serviceflag", tr("收服务费"));
    tablemodel->save_set_header("item_flag", tr("库存管理"));
    tablemodel->save_set_header("ch_deductflag", tr("服务提成"));
    tablemodel->save_set_header("int_rate", tr("税率设置"));
    ...
    //
    ui->comboBox_filter->addItem(tr("所有商品"), "");
    ui->comboBox_filter->addItem(tr("非原料菜品"), " ifnull(ch_material_flag, '') <> 'Y' ");
    ui->comboBox_filter->addItem(tr("原料菜品"), " ifnull(ch_material_flag, '') = 'Y' ");
    ui->comboBox_filter->addItem(tr("已添加原料"), " ch_dishno in (select ch_dishno from material_bt_dish) ");
    ui->comboBox_filter->addItem(tr("库存管理"), " ifnull(item_flag, '') = 'Y' ");

    ui->comboBox_filter_2->addItem(tr("启用菜"), " ifnull(ch_stopflag, '') <> 'Y' ");
    ui->comboBox_filter_2->addItem(tr("停用菜"), " ifnull(ch_stopflag, '') = 'Y' ");

    //! toolbar
    ui->widget->setup(QStringList()
                      //                      << tr("大类设置")
                      << tr("品类设置") << tr("单位设置") << tr("新增")
                      << tr("修改") << tr("删除") << tr("刷新") << tr("导出")<< tr("导入")
                      << tr("退出")
                      );
    //! connect
    ...
    connect(ui->widget->index_widget(tr("导入")), SIGNAL(clicked()),this,SLOT(toimport()));
    ...
    //! refresh
    tablemodel_arg4 =
            "    if('%1' = '', ch_dish_typeno like '%', ch_dish_typeno = '%1') "
            " and  if('%2' = '', ch_sub_typeno like '%', ch_sub_typeno = '%2') "
...
void w_bt_dish_base::treeselect()
{
    ...
    //检索
    QString filter = ui->comboBox_filter->itemData(ui->comboBox_filter->currentIndex()).toString();
    if(!filter.isEmpty()) filter = " and " + filter;
    QString filter_2 = ui->comboBox_filter_2->itemData(ui->comboBox_filter_2->currentIndex()).toString();
    if(!filter_2.isEmpty()) filter_2 = " and " + filter_2;
    tablemodel->setFilter(tablemodel_arg4.arg(ch_dish_typeno).arg(ch_sub_typeno).arg(vch_spell) + filter + filter_2);
...
void w_bt_dish_base::tonew()
{
    if(ui->treeWidget->deepofitem(ui->treeWidget->currentItem())==2){
        ...
        lds_messagebox::information(this, MESSAGE_TITLE_VOID,tr("请选择菜品小类"));
    }
}
...
void w_bt_dish_base::tochange()
{
    ...
    if(row<0){
        lds_messagebox::information(this, MESSAGE_TITLE_VOID,tr("请选择将要修改的菜品记录"));
        return;
    }
...
void w_bt_dish_base::to_yun_version_diff(int d)
{
    ...
    static_cast<lds_toolbar_button *>(ui->widget->index_widget(tr("退出")))->show_flag_bottom_right(f);
}
    ...
    : QDialog(parent),ui(new Ui_w_bt_dish_typeset_dialog)
{
    ...
    //! toolbar
    ui->widget_2->setup(QStringList()<<tr("新增")<<tr("删除")<<tr("标签批处理")<< tr("保存") << tr("退出"),Qt::AlignRight, lds_keytoolbutton::NOKEY);
    ...
    connect(ui->widget_2->index_widget(tr("标签批处理")),SIGNAL(clicked()),this,SLOT(tolabelselect()));
    ...
    ui->widget->setup(QStringList() << tr("新增") << tr("修改") << tr("删除"), Qt::AlignLeft);
    connect(ui->widget->index_widget(tr("新增")),SIGNAL(clicked()),this,SLOT(tonew()));
    connect(ui->widget->index_widget(tr("修改")),SIGNAL(clicked()),this,SLOT(tochange()));
    connect(ui->widget->index_widget(tr("删除")),SIGNAL(clicked()),this,SLOT(todel()));
    ...
    if(0 == query.recordValue(0).toLongLong()){
        if(!query.execInsert("cey_bt_dish_type",qrtVariantPairList()
                             ...
                             << qrtVariantPair("vch_dish_typename", tr("默认大类")))) {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("操作失败")+ QString::number(__LINE__));
        }
    }
...
void w_bt_dish_base_typeset::todel2()
{
    ...
    if(row >= 0) {
        ...
        if(query.recordValue(0).toLongLong() > 0) {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("该小类下已有菜品，不能删除"));
            return;
        }
...
{
    switch(lds_messagebox::question(this, MESSAGE_TITLE_VOID, _TEXT_SLOT(this), tr("全选"), tr("全不选"), tr("取消"))){
    case 0:
    {
        for(int row = 0, rowcount = tablemodel_2->rowCount(); row < rowcount; row++) {
...
void w_bt_dish_base_typeset::toupdateColor(const QModelIndex &index)
{
    if(index.column() == tablemodel_2->fieldIndex("vch_color")) {
        ...
        if(QDialog::Accepted == lds_roundeddialog_rect_align(&dialog).exec()) {
            ...
            if( 1 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("操作取消"), tr("保存之前的菜品颜色"), tr("清除菜品颜色"))) {
...
void w_bt_dish_base_typeset::tonew()
{
    ...
    if(dialog.ui->lineEdit_2->text().trimmed().isEmpty()) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("大类名称")+tr("不能为空"));
        return;
    }
    //
    lds_query::tran_saction();
...
void w_bt_dish_base_typeset::todel()
{
    ...
    if(1 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("确认删除"), tr("是"), tr("否"))) {
        return;
    }
    ...
    ui(new Ui_w_bt_dish_unitset_Dialog)
{
    ...
    tablemodel->setEditStrategy(lds_model_sqltablemodel::OnManualSubmit);
    tablemodel->save_set_header("ch_unitno",tr("编码"));
    ...
    if(lds::get_soft_curent_language_first() != "EN"){
        ...
        ui->label_desc->setText("["+unitlist+"]"+tr("选择默认单位时,前台称重时会自动匹配对应的单位"));
    }
...
void w_bt_dish_base_unitset::todel()
{
    ...
    if(query.recordValue(0).toLongLong()>0){
        //指定单位已存在菜品记录，不能删除!
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("指定单位已存在菜品记录，不能删除"));
        return;
    }
    if(1 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("确认删除"), tr("是"), tr("否"))) {
        return;
...
bool w_bt_dish_base_unitset::tosave()
{
    ...
    for(int r=0,r_count=tablemodel->rowCount();r<r_count;r++){
        ...
        if(ls_unitno.trimmed().isEmpty()){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("第%1行，编码为空，不能保存").arg(r+1));
            return false;
        }
        if(ls_unitname.trimmed().isEmpty()){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("第%1行，单位为空，不能保存").arg(r+1));
            return false;
        }
    }
    QString errstring;
...
void w_bt_dish_base_change::delayInitData()
{
    ...
    //! connect
    connect(ui->toolbar->index_widget(tr("保存")),SIGNAL(clicked()),this,SLOT(tosave()));
    connect(ui->toolbar->index_widget(tr("保存并新增")),SIGNAL(clicked()),this,SLOT(tocontinue()));
    ...
    connect(ui->toolbar->index_widget(tr("退出")),SIGNAL(clicked()),this,SLOT(toexit()));
    ...
    //! after
    ui->toolbar->index_widget(tr("保存并新增"))->hide();
    ...
    static_cast<QPushButton*>(ui->toolbar->index_widget(tr("保存")))->setDefault(false);static_cast<QPushButton*>(ui->toolbar->index_widget(tr("保存")))->setAutoDefault(false);
    static_cast<QPushButton*>(ui->toolbar->index_widget(tr("保存并新增")))->setDefault(false);static_cast<QPushButton*>(ui->toolbar->index_widget(tr("保存并新增")))->setAutoDefault(false);
    static_cast<QPushButton*>(ui->toolbar->index_widget(tr("退出")))->setDefault(false);static_cast<QPushButton*>(ui->toolbar->index_widget(tr("退出")))->setAutoDefault(false);
    //! TIMESLOT
    mapperupdate();
    updatesubdishtype();
    setTypeEnabled(true);
...
void w_bt_dish_base_new::delayInitData()
{
    ...
    connect(ui->toolbar->index_widget(tr("刷新")),SIGNAL(clicked()),this,SLOT(tokeeprefresh()));
    //! init
...
bool w_bt_dish_base_new::saveData()
{
    ...
    if(tablemodel->trySubmitAll()
        ...
        lds_query::roll_back();
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("数据库错误")+tr("或者菜品编码有重复") + QString::number(__LINE__));
        return false;
    }
}
...
bool w_bt_dish_base_dish_type::tosave()
{
    ...
    //            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("大类编号必须为2位"));
    //            return false;
    //        }
    //    }
    ...
    : QDialog(parent)
{
    ...
    ui->setupUi(this);
    resize(800, 560);
    ui->tableView_pic->updateFixed(1,1, 100, 120);
...
void w_bt_dish_base_new_virtual::delayInitData()
{
    ...
    model_dish->selectionmodel = ui->tableView_pic->selectionModel();
    model_dish_d = new ftableview_delegate(ui->tableView_pic);
    model_dish_d->indexisEmptystillDraw = true;
    ...
    querymodel.setQuery("select ch_unitno, vch_unitname from cey_bt_unit ");
    backheader::ldsComSetdataByKVM(&querymodel, ui->comboBox_vch_pur_unitno);
    backheader::ldsComSetdataByKVM(&querymodel, ui->comboBox_ch_unitno);

    backheader::ldsComSetdataByKVM(&w_bt_dish::get_MODEL_DISH_DEDUCTTYPE(),ui->comboBox_ch_deducttype);
    querymodel.setQuery("select ch_dish_typeno, vch_dish_typename from cey_bt_dish_type ");
    backheader::ldsComSetdataByKVM(&querymodel, ui->comboBox_ch_dish_typeno);
    querymodel.setQuery("select ch_sub_typeno, vch_sub_typename from cey_bt_dish_subtype ");
    backheader::ldsComSetdataByKVM(&querymodel, ui->comboBox_ch_sub_typeno);
    //3
    dish_item_model = 0;
    //
    ...
    //! toolbar
    ui->toolbar->setup(QStringList()<<tr("上一行")<<tr("下一行") << tr("保存")<<tr("保存并新增")<<tr("退出"), Qt::AlignRight);
...
{
    QSqlQueryModel querymodel;
    querymodel.setQuery(QString("select ch_sub_typeno, vch_sub_typename from cey_bt_dish_subtype where ch_dish_typeno = '%1' ")
                        .arg(ch_dish_typeno));
    backheader::ldsComSetdataByKVM(&querymodel, ui->comboBox_ch_sub_typeno);
}
...
void w_bt_dish_base_new_virtual::addmapping(lds_model_sqltablemodel *model, QDataWidgetMapper *mapper)
{
    ...
    mapper->addMapping(ui->lineEdit_num_item_warn,model->fieldIndex(("num_item_warn")));
...
bool w_bt_dish_base_new_virtual::saveCheck()
{
    ...
    if(!ui->lineEdit_vch_barcode->text().isEmpty()){
        ...
        query.next();
        if(query.recordValue(0).toLongLong() > 0) {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("条码不能重复"));
            return false;
        }
    }
    //开头不能为0
    if(ui->lineEdit_ch_dishno->text().startsWith("0")){
        if(1 == lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("编号开头为0，若使用点菜宝则禁止编号开头为0，是否继续"), tr("继续"), tr("取消"))){
            return false;
        }
    }
    if(ui->lineEdit_ch_dishno->text().length() != 5) {
        if(1 == lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("编号长度不足5位，若使用点菜宝则长度必须为5位，是否继续"), tr("继续"), tr("取消"))){
            return false;
        }
    }
    if(ui->lineEdit_ch_dishno->text().isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("菜品编码不能为空"));
        return false;
    }
    if(b_enable_material
            && ui->comboBox_ch_dish_typeno->comusrdata().toString().isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("菜品大类不能为空"));
        return false;
    }
    if(b_enable_material
            && ui->comboBox_ch_sub_typeno->comusrdata().toString().isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("菜品小类不能为空"));
        return false;
    }
    if(ui->lineEdit_vch_dishname->text().isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("菜品名称不能为空"));
        return false;
    }
    if(ui->comboBox_ch_unitno->currentText().isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("菜品单位不能为空"));
        return false;
    }
    if(ui->lineEdit_ch_groupno->text().trimmed().isEmpty()) {
        if(1 == lds_messagebox::question(this, MESSAGE_TITLE_VOID,tr("组号不能为空，将改为菜品编码？"), tr("确认"), tr("取消"))){
            return false;
        }
    ...
    if(query.recordValue(0).toInt() == 0) {
        if(ui->lineEdit_ch_groupno->text() != ui->lineEdit_ch_dishno->text()) {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("组号必须为菜品编码之一"));
            return false;
        }
    }
    ...
    if(dish_item_model) {
        for(int row  = 0; row < dish_item_model->rowCount(); row++) {
            if(dish_item_model->opera(row) != lds_model_sqltablemodel::OpDelete
                ...
                lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("原料为空") +","+tr("操作失败"));
                return false;
            }
        }
    }
    if(ui->checkBox_item_flag->isChecked() && ui->spinBox_int_unit_rate->value() == 0) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, ui->label_int_unit_rate->text() + tr("不能等于零"));
        return false;
    }
    ...
    if(ui->checkBox_ch_material_flag->isChecked() && material_has_be_added) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("原料不能添加原料") + "," + tr("操作失败"));
        return false;
    }
    if(ui->checkBox_ch_weight->isChecked()
            && ui->checkBox_ch_count->isChecked()) {
        if(ui->doubleSpinBox_num_count_weight->value() == 0) {
            lds_messagebox::warning(this ,MESSAGE_TITLE_VOID, ui->checkBox_ch_count->text() +  tr("不能等于零"));
            return false;
        }
    }
    return true;
...
void w_bt_dish_base_new_virtual::toshow_group_info()
{
    ...
    lds_messagebox::information(this, MESSAGE_TITLE_VOID,  tr("当前组号信息") + "\n" +  ch_groupno_info);
}
...
void w_bt_dish_base_new_virtual::totab_item_set()
{
    ...
    if(dish_item_model == 0) {
        ...
        dish_item_model->save_set_header(dish_item_model->fieldIndex("ch_itemno"), tr("原料"));
        dish_item_model->save_set_header(dish_item_model->fieldIndex("num_num"), tr("数量"), true);
    ...
    if(!query.next()) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("请先保存菜品"));
...
void w_bt_dish_base_new_virtual::frame_setEnable(bool e)
{
    ...
    ui->lineEdit_num_item_warn->setEnabled(e);
...
void w_bt_dish_base_new_virtual::toSaaSCheck(bool check)
{
    ...
    if(ui->checkBox_ch_suitflag == this->sender()
            && !saas_pay_detail::SaaSFuncCheck(this, "0401"/*套菜*/)) {
    ...
    if(ui->checkBox_ch_material_flag == this->sender()
            && !saas_pay_detail::SaaSFuncCheck(this, "0403"/*菜品原料*/)) {
        qobject_cast<QCheckBox *>(this->sender())->setChecked(false);
        return;
    }
    if(ui->checkBox_ch_curprice == this->sender()
            && !saas_pay_detail::SaaSFuncCheck(this, "0404"/*菜品时价菜*/)) {
        qobject_cast<QCheckBox *>(this->sender())->setChecked(false);
        return;
    }
    if(ui->checkBox_ch_tempflag == this->sender()
            && !saas_pay_detail::SaaSFuncCheck(this, "0405"/*菜品临时菜*/)) {
        qobject_cast<QCheckBox *>(this->sender())->setChecked(false);
        return;
    }
    if(ui->checkBox_ch_guqing_flag == this->sender()
            && !saas_pay_detail::SaaSFuncCheck(this, "0406"/*菜品沽清*/)) {
        qobject_cast<QCheckBox *>(this->sender())->setChecked(false);
        return;
    }
    if(ui->checkBox_ch_serviceflag == this->sender()
            && !saas_pay_detail::SaaSFuncCheck(this, "0407"/*菜品服务费*/)) {
        qobject_cast<QCheckBox *>(this->sender())->setChecked(false);
        return;
    }
    if(ui->checkBox_ch_deductflag == this->sender()
            && !saas_pay_detail::SaaSFuncCheck(this, "0408"/*菜品营业员提成*/)) {
        qobject_cast<QCheckBox *>(this->sender())->setChecked(false);
        return;
    }
}
...
void w_bt_dish_base_new_virtual::showStackedIndex(int index)
{
    switch(index) {
    case 0:
        lds::showWidget(ui->frame_basic);ui->frame_basic->move(20, 50);
...
void w_bt_dish_base_new_virtual::toSetCountWeight(bool check)
{
    ...
    if(ui->checkBox_ch_weight == this->sender() && check) {
        if(0 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("启用称重计数"), tr("是"), tr("否"))) {
            ...
            if(dialog.windowTitle().isEmpty()) {
                dialog.setWindowTitle(tr("没有类型"));
            }
            dialog.setWindowTitle(tr("计价秤") + ":" + dialog.windowTitle());
            dialog.ui->label->setText(tr("取重") + "(G)");
            //
            ElectronicCountPrice jijia(0);
            connect(&jijia,SIGNAL(weighCommingx1000(double)),dialog.ui->lineEdit,SLOT(setValue(double)));
            jijia.setType(lds::sysconf->value("system_setting/dianzichengxinghao").toString());
            jijia.setBaudrate(lds::sysconf->value("system_setting/dianzicheng_baudrate").toInt());
            if(!jijia.tryStart_qs(lds::sysconf->value("system_setting/dianzichengduankou").toString())) {
                //               lds_messagebox::warning(this, MESSAGE_TITLE_VOID, jijia.lastError());
            }
            //
            if(QDialog::Accepted == lds_roundeddialog_rect_align(&dialog).exec()){
...
void w_bt_dish_base_new_virtual::toupdateDishImage()
{
    if(imagedishdir.isEmpty() ) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("菜品图片(文件夹)")+tr("为空"));
        return;
    }
    ...
    if(!ui->lineEdit_ch_dishno->isEnabled()){
        QString filepath = getFilePath::getOpenFileName(this, tr("选择图片"),
            ...
            if(!p.isNull()){
                ...
                for(int begin = 0; begin < files.count();begin++){
                    if(files[begin].startsWith(curimagebaseName+".")){
                        ...
                        if(!file.remove()){
                            lds_messagebox::information(this, MESSAGE_TITLE_VOID,tr("清除文件失败")+file.errorString());
                            return;
                        }
                    }
                }
...
{
    switch(lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("清除"), tr("清除菜品图片"), tr("清除菜品颜色"), tr("取消"))) {
    case 0://清除菜品图片
    {
...
void w_bt_dish_base_change_by_ch_dishno::delayInitData()
{
    ...
    ui->toolbar->index_widget(tr("保存并新增"))->setVisible(false);
