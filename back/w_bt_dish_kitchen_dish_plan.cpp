    ...
    ui(new Ui::w_bt_dish_kitchen_dish_plan)
{
    ...
    tablemodel->save_set_header("vch_plan_id", tr("方案编号"));
    tablemodel->save_set_header("vch_plan_name", tr("方案名称"));
    tablemodel->save_set_header("int_print_id", tr("打印机"));
    tablemodel->save_set_header("ch_print_id_second_use", tr("启用备用打印机"));
    tablemodel->save_set_header("int_print_id_second", tr("备用打印机"));

    tablemodel->save_set_header("int_dish_type", tr("单据类型"));
    tablemodel->save_set_header("int_suit_type", tr("套菜类型"));
    tablemodel->save_set_header("int_print_count", tr("份数"));
    tablemodel->save_set_header("int_head_block", tr("票头留白"));
    tablemodel->save_set_header("int_tail_block", tr("票尾留白"));

    /*tablemodel->save_set_header("vch_plan_title", tr("自定义表头"));
    tablemodel->save_set_header("vch_plan_tail", tr("自定义票尾"));*/
    tablemodel->save_set_header("ch_check_state", tr("判断打印机状态"));
    tablemodel->save_set_header("vch_dishname_No", tr("切换菜名"));

    tablemodel->tablename_kvmap_insert("int_dish_type", "0", tr("一桌一单"));
    tablemodel->tablename_kvmap_insert("int_dish_type", "1", tr("一大类一单"));
    tablemodel->tablename_kvmap_insert("int_dish_type", "2", tr("一小类一单"));
    tablemodel->tablename_kvmap_insert("int_dish_type", "3", tr("一菜一单"));

    tablemodel->tablename_kvmap_insert("int_suit_type", "0", tr("打印在一起"));
    tablemodel->tablename_kvmap_insert("int_suit_type", "1", tr("只打印主套菜"));
    tablemodel->tablename_kvmap_insert("int_suit_type", "2", tr("只打印套菜明细"));

    tablemodel->tablename_kvmap_insert("cey_bt_kitchen_print", "int_id", "vch_printname", "", true, lds_model_sqltablemodel::SELECT_AUTO_REFRESH_KEY);

    tablemodel->tablename_kvmap_insert("vch_dishname_No", "vch_dishname", tr("菜品名称")+"1");
    tablemodel->tablename_kvmap_insert("vch_dishname_No", "vch_dishname_2", tr("菜品名称")+"2");
    //
...
void w_bt_dish_kitchen_dish_plan::todel()
{
    ...
    if(0 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("确认删除"), tr("是"), tr("否"))) {
...
bool w_bt_dish_kitchen_dish_plan_new_virtual::saveCheck()
{
    ...
    //        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("编号必须为2位"));
    //        return false;
    //    }
    if(ui->lineEdit_vch_plan_name->text().isEmpty()) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("名称不能为空"));
        return false;
    }
    if(ui->comboBox_int_print_id->currentText().isEmpty()) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("打印机为空"));
        return false;
    }
    if(ui->comboBox_int_print_id_second->itemData(ui->comboBox_int_print_id_second->currentIndex()).toString()
        ...
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("备用打印机和打印机不能相同"));
        return false;
    }
    return true;
}
    ...
    w_bt_dish_kitchen_dish_plan_new_virtual(parent)
{
    ...
    ui->lineEdit_vch_plan_name->setText(tr("方案")+ui->lineEdit_vch_plan_id->text());
    ...
    ui->frame->index_widget(tr("退出"))->setFocus();
    connect(ui->frame->index_widget(tr("保存")), SIGNAL(clicked()),this,SLOT(tosave()));
    connect(ui->frame->index_widget(tr("退出")), SIGNAL(clicked()),this,SLOT(toexit()));
