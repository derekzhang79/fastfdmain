    ...
    ui(new Ui::w_bt_dish_kitchen_dish_state)
{
    ...
    tablemodel->save_set_header("ch_dishno", tr("菜品"));

    tablemodel->save_set_header("ch_billno", tr("单号"));
    //强制执行select
...
void w_bt_dish_kitchen_dish_state::tolostclear()
{
    ...
    if(ui->radioButton_lost->isChecked()) {//清除漏打单
        ...
        if(0 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, _TEXT_SLOT(this), tr("确定"), tr("取消"))) {
            /** */ lds_query::tran_saction();
            //清除 未厨打的 的bug
...
QString w_bt_dish_kitchen_dish_state::get_state_desc(const QByteArray &state, int index)
{
    ...
    //
    return tr("正在") + "\"" +w_bt_dish::k2o3_name(key)+"\"";
}
