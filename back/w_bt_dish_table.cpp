...
{
    ui->setupUi(this);
    ui->label->setText(tr("检索"));
    ui->lineEdit->setPlaceholderText(tr("餐桌号")+","+tr("餐桌名"));
    ...
    tablemodel->save_set_header("ch_tableno", tr("餐桌号"));
    tablemodel->save_set_header("vch_tablename", tr("餐桌名称"));
    tablemodel->save_set_header("ch_typeno", tr("餐桌类型"));
    tablemodel->save_set_header("int_person", tr("人数"));
    tablemodel->save_set_header("ch_areano", tr("所属区域"));
    ui->tableView->setEditTriggers(QTableView::NoEditTriggers);
    ui->tableView->setModel(tablemodel);
    ui->tableView->rePecifyHeader(tablemodel->get_save_set_header_saver_column_list());

    tablemodel->tablename_kvmap_insert("cey_bt_table_area", "ch_areano", "vch_areaname", "", false, lds_model_sqltablemodel::SELECT_AUTO_REFRESH_KEY);
    tablemodel->tablename_kvmap_insert("cey_bt_table_type", "ch_typeno", "vch_typename", "", false, lds_model_sqltablemodel::SELECT_AUTO_REFRESH_KEY);
...
bool w_bt_dish_table::tableDelTransaction(const QString &ch_tableno, QString *errstring)
{
    ...
    if(query.recordValue(0).toInt()>0){
        *errstring  = tr("指定餐桌记录已经营业操作,不能删除");
        return false;
    }
...
void w_bt_dish_table::tonew()
{
    ...
    if(ui->treeWidget->deepofitem(ui->treeWidget->currentItem())==1){
        ...
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("请先选择餐桌类型"));
    }
}
...
void w_bt_dish_table::todel()
{
    int row=ui->tableView->currentIndex().row();
    if(row<0){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("请选择将要删除的餐桌记录"));
        return;
    }
    ...
    : QDialog(parent)
{
    ...
    //! toolbar
    ui->widget->setup(QStringList()<<tr("上一行")<<tr("下一行")<<tr("保存")<<tr("退出"), Qt::AlignRight, lds_keytoolbutton::SYSKEY);
    ...
    ui->widget->setDefaultFalseAutoDefaultFalse(tr("上一行"));
    ui->widget->setDefaultFalseAutoDefaultFalse(tr("下一行"));
    ui->widget->setDefaultFalseAutoDefaultFalse(tr("保存"));
    ui->widget->setDefaultFalseAutoDefaultFalse(tr("继续"));
    ui->widget->setDefaultFalseAutoDefaultFalse(tr("退出"));
    //! after
...
void w_bt_dish_table_news::togenrate()
{
    ...
    if(!ok) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, ui->label_code_start->text() + "," + tr("必须为数字"));
        return;
    }
    ui->lineEdit_code_end->text().toInt(&ok);
    if(!ok) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, ui->label_code_end->text() +  "," + tr("必须为数字"));
        return;
    }
    ...
    _parent(static_cast<w_bt_dish_table* >(parent))
{
    ui->setupUi(this);
    //! toolbar
    ui->widget->setup(QStringList()<<tr("新增")<<tr("删除")<<tr("保存")<<tr("退出"), Qt::AlignRight, lds_keytoolbutton::SYSKEY);
    //! tableview
    backheader::ldsComSetdataByKVM(&w_bt_dish::get_MODEL_DISH_LOW_MODE(), ui->comboBox_ch_low_mode);
    backheader::ldsComSetdataByKVM(&w_bt_dish::get_MODEL_DISH_SERVICE_MODE(), ui->comboBox_ch_service_mode);
    backheader::ldsComSetdataByKVM(&w_bt_dish::get_MODEL_TABLE_SERVICE_FLAG_MODE(), ui->comboBox_ch_serviceflag);
    ...
    tablemodel->save_set_header("vch_typename",  tr("桌类名称"));
    ui->tableView->setModel(tablemodel);
    ui->tableView->setEditTriggers(QTableView::NoEditTriggers);
...
void w_bt_dish_table_tableset::todel()
{
    ...
    query.next();
    if(query.recordValue(0).toInt() == 0) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,  tr("该类别下已有餐桌，不能删除"));
        return;
    }
...
bool w_bt_dish_table_tableset::tosave()
{
    ...
    if(tablemodel->rowCount() == 0) {
        lds_messagebox::warning(this ,MESSAGE_TITLE_VOID, tr("请先新增餐桌类型"));
        return false;
    }
    for(int row=0,rowcount=tablemodel->rowCount();row<rowcount;row++){
        if(tablemodel->model_data(row, "ch_typeno").toString().isEmpty()){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("%1行 编码不能为空").arg(row+1));
            return false;
        }
        if(tablemodel->model_data(row, "vch_typename").toString().isEmpty()){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("%1行 桌类名称不能为空").arg(row+1));
            return false;
        }

        if(tablemodel->model_data(row, "ch_service_mode").toString().isEmpty()){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("%1行 服务费模式不能为空").arg(row+1));
            return false;
        }

        if(tablemodel->model_data(row, "ch_low_mode").toString().isEmpty()){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("%1行 低消方式不能为空").arg(row+1));
            return false;
        }
        ...
        if(tablemodel->index(row, tablemodel->fieldIndex("ch_service_mode")).data().toInt() == 1
            ...
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("%1行 超钟收费单位不能小于等于0").arg(row+1));
            return false;
        }
        //        if(tablemodel->index(row, tablemodel->fieldIndex("ch_service_mode")).data().toInt() == 3
        //                &&tablemodel->index(row, tablemodel->fieldIndex("num_max_service")).data().toInt()<
        //                tablemodel->index(row, tablemodel->fieldIndex("num_ration")).data().toInt()
        //                ){
        //            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("%1行 定额服务费不能高于最高服务费").arg(row+1));
        //            return false;
        //        }
    ...
    ui(new Ui_w_bt_dish_unitset_Dialog)
{
    ...
    tablemodel->setEditStrategy(lds_model_sqltablemodel::OnManualSubmit);
    tablemodel->save_set_header(tablemodel->fieldIndex("ch_areano"), tr("编码"));
    tablemodel->save_set_header(tablemodel->fieldIndex("vch_areaname"), tr("区域名称"), 1);
...
void w_bt_dish_table_areaset::todel()
{
    ...
    if(query.recordValue(0).toLongLong()>0){
        //指定单位已存在菜品记录，不能删除!
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("指定区域已存在餐桌记录，不能删除"));
        return;
    }
...
bool w_bt_dish_table_areaset::tosave()
{
    ...
    for(int r=0,r_count=tablemodel->rowCount();r<r_count;r++){
        ...
        if(ls_areano.trimmed().isEmpty()){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("第%1行，区域代码为空，不能保存").arg(r+1));
            return false;
        }
        if(ls_areaname.trimmed().isEmpty()){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("第%1行，区域名称为空，不能保存").arg(r+1));
            return false;
        }

        if(findareano(ls_areano, r)!=-1){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("区域代码有重复，请核实"));
            return false;
        }
    }

    if(tablemodel->trySubmitAll()){
        lds_messagebox::information(this, MESSAGE_TITLE_VOID,tr("数据保存成功"));
        return true;
    } else {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("数据库错误"));
        return false;
    ...
    : QDialog(parent)
{
    ...
    tablemodel->setTable("cey_bt_table_type");
    tablemodel->setEditStrategy(lds_model_sqltablemodel::OnManualSubmit);
    tablemodel->save_set_header("ch_typeno", tr("编码"));
    tablemodel->save_set_header("vch_typename", tr("桌类名称"), 1);
    //    tablemodel->save_set_header("num_max_service", tr("最高服务费"), 1);

    bool pay_fixed_service = n_func::f_get_sysparm("pay_fixed_service", "0") == "1";
    tablemodel->save_set_header("ch_service_mode", tr("服务费模式"), pay_fixed_service?0:1);
    tablemodel->save_set_header("num_ration", tr("服务费") + "%", pay_fixed_service?0:2);

    tablemodel->save_set_header("ch_low_mode", tr("低消方式"), 1);
    tablemodel->save_set_header("num_lowcost", tr("低消金额"), 1);
...
bool w_bt_dish_table_new2::tosave()
{
    ...
    query.next();
    if(query.recordValue(0).toInt() > 0) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("餐桌号有重复"));
    }
...
{
    if(0 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("确认删除"), tr("确定"), tr("取消")) ) {
        QString errstring;
...
bool w_bt_dish_table_virtual::saveCheck()
{
    ...
    if(ui->comboBox_ch_typeno->currentIndex() < 0){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("餐桌类型不能为空"));
        return false;
    }
    if(ui->comboBox_ch_areano->currentIndex() < 0){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("餐区不能为空"));
        return false;
    }

    if(ch_tableno.startsWith("0")){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("编号开头不能为0"));
        return false;
    }
    if(ch_tableno.toInt() == 0){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("编号必须为数字"));
        return false;
    }
    if(ch_tableno.trimmed().isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("餐桌号不能为空"));
        return false;
    }
    if(ui->lineEdit_vch_tablename->text().trimmed().isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("餐桌名不能为空"));
        return false;
    }
    return true;
}
    ...
    : QDialog(parent)
{
    ...
    tablemodel->setEditStrategy(lds_model_sqltablemodel::OnManualSubmit);
    tablemodel->save_set_header("ch_tableno",tr("餐桌号"));
    tablemodel->save_set_header("vch_tablename",tr("餐桌名"), true);
    tablemodel->save_set_header("ch_typeno",tr("餐桌类型"), true);
    tablemodel->save_set_header("ch_areano",tr("餐桌区域"), true);
    tablemodel->save_set_header("int_person",tr("人数"), true);
