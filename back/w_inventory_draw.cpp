    ...
    : QDialog(parent)
{
    ...
    //    tablemodel_detail->save_set_header("ch_sheetno", tr("单据号码"));
    tablemodel_detail->save_set_header("ch_materialno", tr("商品名称"));
    tablemodel_detail->save_set_header("num_sys", tr("系统库存"));
    tablemodel_detail->save_set_header("num_check", tr("盘点数量"), true);
    tablemodel_detail->save_set_header("num_pro_loss", tr("盈亏数量"));
    //    tablemodel_detail->save_set_header("cost_price", tr("进价"));
    tablemodel_detail->save_set_header("chg_flag", tr("调整库存"), true);
