...
//会员卡积分类型
QStandardItemModel w_m_member::VIP_MODEL_POINT_CH_TYPE;
//会员付款方式
...
QStandardItemModel &w_m_member::get_VIP_MODEL_FAVOUR()
{
    ...
    if(VIP_MODEL_FAVOUR.rowCount() == 0) {
        items.clear();items<<new QStandardItem("1")<<new QStandardItem(tr("会员特价"));VIP_MODEL_FAVOUR.appendRow(items);
        items.clear();items<<new QStandardItem("2")<<new QStandardItem(tr("固定折扣"));VIP_MODEL_FAVOUR.appendRow(items);
    }
    return VIP_MODEL_FAVOUR;
}

QStandardItemModel &w_m_member::get_VIP_MODEL_M_PRICE()
{
    QList<QStandardItem *> items;
    if(VIP_MODEL_M_PRICE.rowCount() == 0) {
        items.clear();items<<new QStandardItem("1")<<new QStandardItem(tr("会员特价1"));VIP_MODEL_M_PRICE.appendRow(items);
...
QStandardItemModel &w_m_member::get_VIP_MODEL_POINT_CH_TYPE()
{
    QList<QStandardItem *> items;
    if(VIP_MODEL_POINT_CH_TYPE.rowCount() == 0) {
        items.clear();items<<new QStandardItem("3")<<new QStandardItem(tr("消费积分"));VIP_MODEL_POINT_CH_TYPE.appendRow(items);
        items.clear();items<<new QStandardItem("4")<<new QStandardItem(tr("礼品兑换"));VIP_MODEL_POINT_CH_TYPE.appendRow(items);
        items.clear();items<<new QStandardItem("5")<<new QStandardItem(tr("手工充积分"));VIP_MODEL_POINT_CH_TYPE.appendRow(items);
        items.clear();items<<new QStandardItem("6")<<new QStandardItem(tr("手工减积分"));VIP_MODEL_POINT_CH_TYPE.appendRow(items);
...
QStandardItemModel &w_m_member::get_RECHARGE_MODEL_PAYMODE()
{
    ...
    if(RECHARGE_MODEL_PAYMODE.rowCount() == 0) {
        items.clear();items<<new QStandardItem("0")<<new QStandardItem(tr("会员付款"));RECHARGE_MODEL_PAYMODE.appendRow(items);
        items.clear();items<<new QStandardItem("1")<<new QStandardItem(tr("现金"));RECHARGE_MODEL_PAYMODE.appendRow(items);
        items.clear();items<<new QStandardItem("2")<<new QStandardItem(tr("银行转帐"));RECHARGE_MODEL_PAYMODE.appendRow(items);
        items.clear();items<<new QStandardItem("3")<<new QStandardItem(tr("银行支票"));RECHARGE_MODEL_PAYMODE.appendRow(items);
        items.clear();items<<new QStandardItem("5")<<new QStandardItem(tr("系统付款"));RECHARGE_MODEL_PAYMODE.appendRow(items);
        items.clear();items<<new QStandardItem("6")<<new QStandardItem(tr("微信"));RECHARGE_MODEL_PAYMODE.appendRow(items);
        items.clear();items<<new QStandardItem("7")<<new QStandardItem(tr("支付宝"));RECHARGE_MODEL_PAYMODE.appendRow(items);
        items.clear();items<<new QStandardItem("8")<<new QStandardItem(tr("移动支付"));RECHARGE_MODEL_PAYMODE.appendRow(items);

        items.clear();items<<new QStandardItem("9")<<new QStandardItem(tr("其他1"));RECHARGE_MODEL_PAYMODE.appendRow(items);
        items.clear();items<<new QStandardItem("10")<<new QStandardItem(tr("其他2"));RECHARGE_MODEL_PAYMODE.appendRow(items);
        items.clear();items<<new QStandardItem("11")<<new QStandardItem(tr("其他3"));RECHARGE_MODEL_PAYMODE.appendRow(items);
...
QStandardItemModel &w_m_member::get_RECHARGE_MODEL_PAYMODE_RECHARGE()
{
    ...
    if(RECHARGE_MODEL_PAYMODE_RECHARGE.rowCount() == 0) {
        items.clear();items<<new QStandardItem("1")<<new QStandardItem(tr("现金"));RECHARGE_MODEL_PAYMODE_RECHARGE.appendRow(items);
        items.clear();items<<new QStandardItem("2")<<new QStandardItem(tr("银行转帐"));RECHARGE_MODEL_PAYMODE_RECHARGE.appendRow(items);
        items.clear();items<<new QStandardItem("3")<<new QStandardItem(tr("银行支票"));RECHARGE_MODEL_PAYMODE_RECHARGE.appendRow(items);
        if(lds::get_soft_curent_language_first() != "EN"){
            items.clear();items<<new QStandardItem("6")<<new QStandardItem(tr("微信"));RECHARGE_MODEL_PAYMODE_RECHARGE.appendRow(items);
            items.clear();items<<new QStandardItem("7")<<new QStandardItem(tr("支付宝"));RECHARGE_MODEL_PAYMODE_RECHARGE.appendRow(items);
            items.clear();items<<new QStandardItem("8")<<new QStandardItem(tr("移动支付"));RECHARGE_MODEL_PAYMODE_RECHARGE.appendRow(items);
        }
        items.clear();items<<new QStandardItem("9")<<new QStandardItem(tr("其他1"));RECHARGE_MODEL_PAYMODE_RECHARGE.appendRow(items);
        items.clear();items<<new QStandardItem("10")<<new QStandardItem(tr("其他2"));RECHARGE_MODEL_PAYMODE_RECHARGE.appendRow(items);
        items.clear();items<<new QStandardItem("11")<<new QStandardItem(tr("其他3"));RECHARGE_MODEL_PAYMODE_RECHARGE.appendRow(items);
    }
...
QStandardItemModel &w_m_member::get_RECHARGE_MODEL_PAYMODE_FETCH()
{
    ...
    if(RECHARGE_MODEL_PAYMODE_FETCH.rowCount() == 0) {
        items.clear();items<<new QStandardItem("1")<<new QStandardItem(tr("现金"));RECHARGE_MODEL_PAYMODE_FETCH.appendRow(items);
        items.clear();items<<new QStandardItem("2")<<new QStandardItem(tr("银行转帐"));RECHARGE_MODEL_PAYMODE_FETCH.appendRow(items);
        items.clear();items<<new QStandardItem("3")<<new QStandardItem(tr("银行支票"));RECHARGE_MODEL_PAYMODE_FETCH.appendRow(items);
...
QStandardItemModel &w_m_member::get_RECHARGE_MODEL_CH_DEPOSIT_MODE()
{
    ...
    if(RECHARGE_MODEL_CH_DEPOSIT_MODE.rowCount() == 0) {
        items.clear();items<<new QStandardItem("1")<<new QStandardItem(tr("常规充值"));RECHARGE_MODEL_CH_DEPOSIT_MODE.appendRow(items);
        items.clear();items<<new QStandardItem("2")<<new QStandardItem(tr("开卡充值"));RECHARGE_MODEL_CH_DEPOSIT_MODE.appendRow(items);
        items.clear();items<<new QStandardItem("3")<<new QStandardItem(tr("付款还现"));RECHARGE_MODEL_CH_DEPOSIT_MODE.appendRow(items);
        items.clear();items<<new QStandardItem("4")<<new QStandardItem(tr("反结帐单"));RECHARGE_MODEL_CH_DEPOSIT_MODE.appendRow(items);
        items.clear();items<<new QStandardItem("5")<<new QStandardItem(tr("转帐充值"));RECHARGE_MODEL_CH_DEPOSIT_MODE.appendRow(items);
        items.clear();items<<new QStandardItem("6")<<new QStandardItem(tr("退卡还现"));RECHARGE_MODEL_CH_DEPOSIT_MODE.appendRow(items);
        items.clear();items<<new QStandardItem("8")<<new QStandardItem(tr("取款"));RECHARGE_MODEL_CH_DEPOSIT_MODE.appendRow(items);
        items.clear();items<<new QStandardItem("9")<<new QStandardItem(tr("付款"));RECHARGE_MODEL_CH_DEPOSIT_MODE.appendRow(items);
...
QStandardItemModel & w_m_member::get_VIP_MODEL_STATE()
{

    QList<QStandardItem *> items;
    if(VIP_MODEL_STATE.rowCount() == 0) {
