...
void w_m_member_cardic::to_read_card()
{
    ...
    ui->lineEdit_ch_state->setText(backheader::VIPFINDVALUE(w_m_member::get_VIP_MODEL_STATE(), lst_member.as_m_state));
...
void w_m_member_cardic::to_clear()
{
    ...
    if(ui->lineEdit_memberno->text().trimmed().isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("卡号为空请先读卡"));
        return;
    }
    if(lst_member.as_m_state=="1" &&  lst_card.amount > 0){//4
        if(0==lds_messagebox::question(this, MESSAGE_TITLE_VOID,tr("此会员状态正常且尚有余额,你确定要初始化这张卡么?"), tr("确定"), tr("取消"))){//3
        } else {
            return;
        }
    }
...
//failure:
    lds_query::roll_back();
    lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("清除失败")+errtext);
    goto end;
success:
    lds_query::com_mit();
    lds_messagebox::information(this, MESSAGE_TITLE_VOID,tr("清除成功"));
end:
    ui->lineEdit_ch_state->clear();
    ui->lineEdit_memberno->clear();
    ui->lineEdit_num_amount->clear();
