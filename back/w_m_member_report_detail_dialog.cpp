    ...
    : QDialog(parent)
{
    ...
    ui->comboBox_vch_operID->addItem(tr("所有"), "");
    //
    ui->label_desc->setText(tr("从") + dtFrom.toString(yyyyMMddhhmmss) + tr("到")+dtFo.toString(yyyyMMddhhmmss));
...
{
    querymodel->setQuery(QString( " select dt_operdate ,"//as `" + tr("时间")+"`," +
                                  " ch_type  ,"//as `" + tr("充值类型")+"`,"+
                                  " ch_pay_mode  ,"//as `" + tr("支付方式")+"`,"+
                                  " num_pay  ,"//as `" + tr("付款金额")+"`,"+
                                  " num_deposit  ,"//as `" + tr("充值金额")+"`,"+
                                  " num_realamount  ,"//as `" + tr("实收金额")+"`,"+
                                  " num_deposit-num_realamount as num_free ,"//as `" + tr("优惠")+"`,"+
                                  " vch_operid  ,"//as `" + tr("操作员")+"`,"+
                                  " vch_memo "//as `" + tr("备注")+"`"+
                                  " from ("
                                  " Select"
