    ...
    ui(new Ui::w_m_member_telComing)
{
    ...
    tablemodel->save_set_header("vch_memberno", tr("会员号"));
    ...
    //
    ui->label->setText(tr("未处理的电话"));

    model_tel = new QStandardItemModel(this);
    model_tel->setHorizontalHeaderLabels(QStringList() << tr("电话") << tr("时间"));
    ...
    if(isQ000) {//读会员直接开台
        ...
        ui->checkBox->setText(ui->pushButton_Q000_mread->text() + "," + tr("删除电话记录"));
    } else {//落单
        ui->pushButton_ok->show();
        ui->checkBox->setText(ui->pushButton_ok->text() + "," + tr("删除电话记录"));
    }
...
void w_m_member_telComing::toorder()
{
    ...
    if(row < 0) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("没有会员被选中"));
        return;
    }
...
void w_m_member_telComing::toTelDel()
{
    ...
    if(1 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("确认删除") + model_tel->item(row, 0)->text(), tr("确定"), tr("取消"))) {
        return;
    }
...
void w_m_member_telComing::toQ000_mread()
{
    ...
    ret_ch_tableno = w_scr_dish_main::Q000;
    ret_vch_memberno = tablemodel->model_data(row, "vch_memberno").toString();
    ret_tel = model_tel->item(ui->tableView_tel->currentIndex().row(), 0)->text();
    this->accept();
}
