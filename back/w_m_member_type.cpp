    ...
    ui(new Ui_w_m_member_type_dialog)
{
    ...
    ui->comboBox_typenames->setView(new lds_tableView);
    ui->comboBox_typenames->setCurrentIndex(-1);


    ui->frame->setup(QStringList()<<tr("读卡") << tr("确定") << tr("取消"),Qt::AlignRight, lds_keytoolbutton::SYSKEY);
...
//    }
    ...
    if(lst_member.as_m_state!="1"){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("此卡状态非正常,不能操作"));
        return;
    }
...
void w_m_member_type::took()
{
    ...
    if(ls_memberno.trimmed().isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr(" 请先读卡"));
        return;
    }
    if(ls_typeno_new.isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("请先选择新类型"));
        return;
    }
    if(ls_typeno==ls_typeno_new){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("会员类型相同"));
        return;
    }
    ...
    case 2:
        if(ls_attribute_new!="2"){
            if(ldec_amount!=0){
                lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("卡余额不为零，充值卡不能转其它类型卡"));
                return;
            }
        }
        break;
    case 3:
        if(ls_attribute_new!="3"){
            if(ldec_amount!=0){
                lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("未结算金额不为零，借记卡不能转其它类型卡"));
                return;
            }
        }
        break;
    }
    if(0==lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("确定当前会员转指定类型?"),tr("确定"),tr("取消"))){
        //!----------------transaction
        ...
        if(ls_err.isEmpty()){
            lds_query::com_mit();
            lds_messagebox::information(this, MESSAGE_TITLE_VOID,tr("会员转类型成功"));
        } else {
            lds_query::roll_back();
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("转类型失败\n")+ls_err);
        }
    }
}
