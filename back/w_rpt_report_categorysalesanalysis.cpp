    ...
    QDialog(parent),ui(new Ui_w_rpt_report_categorysalesanalysis_Dialog)
{
    ...
    ui->comboBox_charttype->addItems(QStringList() << tr("销售数量") << tr("销售金额") << tr("折扣") << tr("赠送金额") << tr("实际金额"));
    //! toolbar
    //    connect(ui->tableView->tableView->horizontalHeader(),SIGNAL(sectionClicked(int)),this,SLOT(toselect()));
    ui->widget->setup(QStringList()<<tr("查询")<< tr("图表分析") <<tr("退出"),Qt::AlignTop);
    ...
    ui->comboBox_dishtype->addItems(QStringList() << tr("小类")<< tr("大类") );
}
...
void w_rpt_report_categorysalesanalysis::tab1sqlinit()
{
    ...
    tab1info.insert(tr("品类销售营业分析"),
                    w_rpt_report::TABTYPE(QStringList()<<tr("菜品类型")<<tr("编码")<<tr("销售数量")<<tr("比例")<<tr("销售金额")<<tr("比例")<<tr("折扣")<<tr("比例")<<tr("赠送金额")<<tr("比例")<<tr("实际金额")<<tr("比例"),
                                          intersql,
                                          QStringList()
                                          <<
    ...
    ui->comboBox->addItem(tr("品类销售营业分析"));
    ////////////
}
...
void w_rpt_report_categorysalesanalysis::toselect()
{
    ...
    lds_query query;
    QString sql = tab1info[tr("品类销售营业分析")].sql
            .arg(ls_begin)
            .arg(ls_end)
            ;
    query.execSelect(tab1info[tr("品类销售营业分析")].filters.at(ui->comboBox_dishtype->currentIndex()).arg(sql));
