    ...
    QDialog(parent),ui(new Ui_w_rpt_report_daymonweekanalysis_Dialog)
{
    ...
    //tabwidget header
    ui->comboBox->addItem(tr("菜品销售统计表"));
...
void w_rpt_report_daymonweekanalysis::modeltranslate_2(const QString &sql, QStandardItemModel *model)
{
    ...
    for(;begin<=end;begin++){
        ...
        if(query.next()){
            ...
            items<<new QStandardItem(query.recordValue("bills").toString());updateheaders(headers, tr("单数"));
            items<<new QStandardItem(query.recordValue("person").toString());updateheaders(headers, tr("人数"));
            items<<new QStandardItem(query.recordValue("tables").toString());updateheaders(headers, tr("餐桌使用数"));
            items<<new QStandardItem(query.recordValue("cost").toString());updateheaders(headers, tr("消费金额"));
            items<<new QStandardItem(query.recordValue("present").toString());updateheaders(headers, tr("赠送金额"));
            items<<new QStandardItem(query.recordValue("discount").toString());updateheaders(headers, tr("折扣"));
            items<<new QStandardItem(query.recordValue("service").toString());updateheaders(headers, tr("服务费"));
            items<<new QStandardItem(query.recordValue("low").toString());updateheaders(headers, tr("低消金额"));
            items<<new QStandardItem(query.recordValue("pay_amt").toString());updateheaders(headers, tr("应收金额"));
