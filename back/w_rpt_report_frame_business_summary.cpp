    ...
    ui(new Ui::w_rpt_report_frame_business_summary)
{
    ...
    ui->tableView_2->lds_tableview_delegate_virtual_enableFieldName2k0v1();

    ui->comboBox->addItem(tr("品项统计"));
    ui->comboBox->addItem(tr("品类统计"));
...
void w_rpt_report_frame_business_summary::show_login_data_by_sql()
{
...
{
    if(false == backheader::operid_right_code(this, "0224")) {
        this->reject();
        return;
    }
    to_query();
...
void w_rpt_report_frame_business_summary::to_query()
{
    QString sql0 = QString(
                ...
                + (is_show_login_data ? qrtEqual(" and b.vch_operid", public_sql::gs_operid) : "")
                )
...
void w_rpt_report_frame_business_summary::to_print()
{
    ...
    w_rpt_report_printFormat *dialog
            = new w_rpt_report_printFormat(w_rpt_report_printFormat::show_model_quick, this->windowTitle(), public_sql::gs_operid, n_func::f_get_sysdatetime_str(), this->metaObject()->className(),
