                          ...
                          , parent)
{
    ...
    ui_help->comboBox_group->addItem(tr("付款方式"), QStringList() << "a.ch_paymodeno");
    ui_help->comboBox_group->addItem(tr("操作员"), QStringList() << "b.vch_operID" << "vch_operator" << "("+backheader::SELECT_OPERID_NAME +")  b");
    ui_help->comboBox_group->addItem(tr("餐桌"), QStringList() << "b.ch_tableno" << "vch_tablename" << "cey_bt_table b");
...
void w_rpt_report_frame_cash::toprint()
{
    ...
    w_rpt_report_printFormat *dialog
            = new w_rpt_report_printFormat(w_rpt_report_printFormat::show_model_quick, ui->comboBox->currentText(), public_sql::gs_operid, n_func::f_get_sysdatetime_str(), QString("%1_%2").arg(this->metaObject()->className()).arg(index),
