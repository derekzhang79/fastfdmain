                          ...
                          , parent)
{
    ...
    ui_help->comboBox_group->addItem(tr("付款单号"), QStringList() << "b.ch_payno");
    ui_help->comboBox_group->addItem(tr("操作员"),  QStringList() << "b.vch_operID" << "vch_operator" << "("+backheader::SELECT_OPERID_NAME +")  b");
    ui_help->comboBox_group->addItem(tr("菜品大类"), QStringList() << "c.ch_dish_typeno" << "vch_dish_typename" << "cey_bt_dish_type c");
    ui_help->comboBox_group->addItem(tr("菜品小类"), QStringList() << "c.ch_sub_typeno" << "vch_sub_typename" << "cey_bt_dish_subtype c");
    ui_help->comboBox_group->addItem(tr("餐桌"), QStringList() << "a.ch_tableno" << "vch_tablename" << "cey_bt_table a");
    ui_help->comboBox_group->addItem(tr("餐桌类型"), QStringList() << "d.ch_typeno" << "vch_typename" << "cey_bt_table_type d");
...
void w_rpt_report_frame_dishsale::toupdate_group2(int index)
{
    ...
    if(!group_list.isEmpty()) {
        q.setQuery(QString("select %1, %2 from %3")
                   .arg(group_list.value(mapKey))
                   .arg(group_list.value(mapValue))
                   .arg(group_list.value(mapTablename))
                   );
    }
    backheader::fillData(ui_help->comboBox_group_2, &q, true, tr("全部"));
}
...
void w_rpt_report_frame_dishsale::toprint()
{
    ...
    w_rpt_report_printFormat *dialog
            = new w_rpt_report_printFormat(w_rpt_report_printFormat::show_model_quick,  ui->comboBox->currentText(),
