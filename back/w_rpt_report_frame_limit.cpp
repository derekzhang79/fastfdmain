...
{
    lds_query query;
    return lds_query::selectValue(query, QString(" select sum(%1)  ").arg(orgName(refName)) + from_sql).toDouble();
}
...
int w_rpt_report_frame_limit::pagecount()
{
    if(_pagecount == -1) {
        lds_query query;
        _pagecount = lds_query::selectValue(query, " select count(0)  " + from_sql).toInt();
    }
    return _pagecount;
}
...
void w_rpt_report_frame_limit::set_onepagecount(int onepagecount)
{
    ...
    qDebug() << "_onepagecount:" << _onepagecount;
}
