    ...
    ui(new Ui::w_rpt_report_frame_material_saleflow2)
{
    ...
    //
    ui->widget->setup(QStringList() << tr("查询") << tr("导出") << tr("打印") << tr("退出") << lds_toolbar::stretch<< tr("分页") << tr("上一页") << tr("下一页"), Qt::AlignLeft);

    connect(ui->widget->index_widget(tr("查询")), SIGNAL(clicked()), this,SLOT(toselect()));
    connect(ui->widget->index_widget(tr("导出")), SIGNAL(clicked()), this,SLOT(toselect()));
    connect(ui->widget->index_widget(tr("打印")), SIGNAL(clicked()), this,SLOT(toselect()));
    connect(ui->widget->index_widget(tr("退出")), SIGNAL(clicked()), this,SLOT(toexit()));

    connect(ui->widget->index_widget(tr("分页")), SIGNAL(clicked()), this,SLOT(toselect()));
    connect(ui->widget->index_widget(tr("上一页")), SIGNAL(clicked()), this,SLOT(toselect()));
    connect(ui->widget->index_widget(tr("下一页")), SIGNAL(clicked()), this,SLOT(toselect()));
}
...
void w_rpt_report_frame_material_saleflow2::toselect()
{
    select_limit.set_onepagecount((ui->tableView->height() //tableview height
                                   ...
                                   - ui->tableView->tableView->verticalHeader()->defaultSectionSize()//tableview lineview height
                                   ) / ui->tableView->tableView->verticalHeader()->defaultSectionSize());
    ...
    if(ui->tableView->tableView->itemDelegateForColumn(querymodel->fieldIndex("ch_dishno")) == 0) {
        ui->tableView->tableView->setItemDelegateForColumn(querymodel->fieldIndex("ch_dishno"),
                                                           new lds_model_sqltablemodel_delegate_com(this, "cey_bt_dish", "ch_dishno", "vch_dishname"));
        ui->tableView->tableView->setItemDelegateForColumn(querymodel->fieldIndex("ch_materialno"),
                                                           ui->tableView->tableView->itemDelegateForColumn(querymodel->fieldIndex("ch_dishno")));
        ui->tableView->tableView->setItemDelegateForColumn(querymodel->fieldIndex("vch_operid"),
