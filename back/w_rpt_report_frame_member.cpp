        ...
        << tr("会员挂账"), parent)
{
    ...
    ///~
    ui_help->comboBox_ch_state->addItem(tr("全部"), "%");
    ui_help->comboBox_ch_state->addItemsByModel(&w_m_member::get_VIP_MODEL_STATE());
    ui_help->comboBox_ch_typeno->addItem(tr("全部"), "%");
    ui_help->comboBox_ch_typeno->addItemsBySql("select ch_typeno, vch_typename from t_m_member_type ");
    ui_help->comboBox_ch_dish_type->addItem(tr("全部"), "%");
...
{
    lds_pushbuttonlock b(ui->widget->index_widget(tr("查询")));
    b.toLock();
    ...
    case 4:///会员菜品消费明细
    {
        ...
        if(tableview->itemDelegateForColumn(model->fieldIndex("vch_print_memo")) == 0) {
            tableview->setItemDelegateForColumn(model->fieldIndex("vch_print_memo"), new lds_model_sqltablemodel_delegate_com(this, "vch_print_memo"));
        }
        //
        ui->pushButton_step->sum_clear(index);
        ui->pushButton_step->sum_append(index,  "num_dish");
