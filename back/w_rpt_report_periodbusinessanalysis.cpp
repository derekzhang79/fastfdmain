...
//    ui->dateTimeEdit->setDateTime(QDateTime(n_func::f_get_sysdatetime().date(), QTime(0,0,0)));
    ...
    ui->comboBox->addItem(tr("时段营业情况分析"));
}
...
//    qlonglong ll_row;
    ...
    if(ui->dateTimeEdit->dateTime() > ui->dateTimeEdit_2->dateTime()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("开始时间不能大于结束时间"));
        return;
    }
...
//    QTime().secsTo(ui->dateTimeEdit_2->dateTime().time())/60;
    ...
    if(ls_stop < ls_start){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("结束时段不能小于起始时段"));
        return;
    }
    if(ll_period<=0){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("时段间隔不能小于或等于零"));
        return;
    }
