    ...
    :QDialog(parent)
{
    ...
    ui->lineEdit_dt_begin->setText(dt_from);
    ui->lineEdit_dt_end->setText(dt_to);
}
    ...
    :QDialog(parent)
{
    ...
    ui->label_dt_begin->setHidden(dt_from.isEmpty());ui->lineEdit_dt_begin->setHidden(dt_from.isEmpty());
    ui->label_dt_end->setHidden(dt_from.isEmpty());ui->lineEdit_dt_end->setHidden(dt_from.isEmpty());
}
...
void w_rpt_report_printFormat::pop(QWidget *basewidget, QWidget *parent)
{
    ...
    if(model == 0
            || model->columnCount() == 0) {
        lds_messagebox::warning(parent, MESSAGE_TITLE_VOID, tr("没有打印数据"));
        return ;
    }
    //
    this->setWindowTitle(MESSAGE_TITLE_VOID);
    lds_roundeddialog_rect_pop d(this, parent);
...
//    ui->checkBox_printspacing_2->setChecked(false);
    ...
    print_format_conf = 0;
    //
    ui->comboBox->addItem(tr("58打印机"),  "userdata/settings/report_format/" + lds::get_soft_curent_language_first() + "/58_" + _conf_path);
    ui->comboBox->addItem(tr("80打印机"),  "userdata/settings/report_format/" + lds::get_soft_curent_language_first() + "/80_" + _conf_path);
    qDebug() << "58:" << ui->comboBox->itemData(0).toString();
    qDebug() << "80:" << ui->comboBox->itemData(1).toString();
    //
    standmodel = new QStandardItemModel(this);
    standmodel->setHorizontalHeaderLabels(QStringList() << tr("选中") << tr("打印项")<< tr("宽度") << tr("行"));
    ui->tableView->setModel(standmodel);
    ui->tableView->setItemDelegateForColumn(1, new lds_tableview_delegate_report(this));
    ui->tableView->setColumnWidth(1, 150);
    ui->tableView->setItemDelegateForColumn(3, new lds_model_sqltablemodel_delegate_com(this, ldsVariantMap("1", 1, "2", 2)));
...
void w_rpt_report_printFormat::toprint()
{
    ...
    for(int row = 0; row < standmodel->rowCount(); row ++) {
        ...
        if(standmodel->item(row, 0)->text() == "Y"){
            if(sqr == 1)
                printline[0].append(line);
            if(sqr == 2)
                printline[1].append(line);
        }
    }
    //表头
    if(ui->checkBox_printtitle->isChecked()){
        ...
        foreach(const printFormat &p, printline[0]) {
            printins.append(backheader::printer.drawText("",
                                                         content_tableview->hh_paint_transfer(p.index),
                                                         p.width));
        }
        if(printline[0].count() > 0)
            printins.append(backheader::printer.drawText("", "\n"));

        foreach(const printFormat &p, printline[1]) {
            printins.append(backheader::printer.drawText("",
                                                         content_tableview->hh_paint_transfer(p.index),
                                                         p.width));
        }
        if(printline[1].count() > 0)
            printins.append(backheader::printer.drawText("", "\n"));
