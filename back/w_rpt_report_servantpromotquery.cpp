    ...
    QDialog(parent),ui(new Ui_w_rpt_report_servantpromotquery_Dialog)
{
    ...
    ui->comboBox_operator->addItem(tr("全部"), "%");
...
void w_rpt_report_servantpromotquery::toselect()
{
    ...
    if(ui->tableView->tableView->itemDelegateForColumn(querymodel->fieldIndex("ch_unitno")) == 0)
        ui->tableView->tableView->setItemDelegateForColumn(querymodel->fieldIndex("ch_unitno"), new lds_model_sqltablemodel_delegate_com(this, "cey_bt_unit", "ch_unitno", "vch_unitname"));
    if(ui->tableView->tableView->itemDelegateForColumn(querymodel->fieldIndex("vch_waiter")) == 0)
        ui->tableView->tableView->setItemDelegateForColumn(querymodel->fieldIndex("vch_waiter"), new lds_model_sqltablemodel_delegate_com(this, ui->comboBox_operator));
...
{
    w_rpt_report_printFormat dialog(w_rpt_report_printFormat::show_model_more, this->windowTitle(), public_sql::gs_operid, n_func::f_get_sysdatetime_str(), this->metaObject()->className(),
