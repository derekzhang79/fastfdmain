    ...
    QDialog(parent),ui(new Ui_w_rpt_report_servicefeelowpinquery_dialog)
{
    ...
    ui->comboBox_2->addItem(tr("免服务费"), 1);
    ui->comboBox_2->addItem(tr("低消"), 2);
    ...
    ui->tableView->tableView->setTransferHheader();
    ui->tableView->lineView->setTransferHheader();
...
void w_rpt_report_servicefeelowpinquery::toselect()
{
    ...
    if(ui->tableView->tableView->itemDelegateForColumn(querymodel->fieldIndex("ch_tableno")) == 0)
        ui->tableView->tableView->setItemDelegateForColumn(querymodel->fieldIndex("ch_tableno"), new lds_model_sqltablemodel_delegate_com(this, "cey_bt_table", "ch_tableno", "vch_tablename"));
    if(ui->tableView->tableView->itemDelegateForColumn(querymodel->fieldIndex("ch_typeno")) == 0)
        ui->tableView->tableView->setItemDelegateForColumn(querymodel->fieldIndex("ch_typeno"), new lds_model_sqltablemodel_delegate_com(this, ui->comboBox));
    if(ui->tableView->tableView->itemDelegateForColumn(querymodel->fieldIndex("ch_type")) == 0)
        ui->tableView->tableView->setItemDelegateForColumn(querymodel->fieldIndex("ch_type"), new lds_model_sqltablemodel_delegate_com(this, ui->comboBox_2));
