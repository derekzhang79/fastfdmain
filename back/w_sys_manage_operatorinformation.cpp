...
{
    ui->setupUi(this);
    ui->label_discount->setText(tr("折扣下限"));
    if(public_sql::discount_off_mode() == "1") {
        ui->label_discount->setText(tr("折扣上限"));
    }
...
void w_sys_manage_operatorinformation::todel()
{
    ...
    if(query.next()) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("有营业数据无法删除"));
        return;
    }
...
bool w_sys_manage_operatorinformation::saveData()
{
    for(int row = 0,rowcount = tablemodel->rowCount();row<rowcount;row++){
        if(tablemodel->model_data(row,  0).toString().startsWith("0")){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("编码不能以0开头，请核实"));
            return false;
        }
    }
    QString errstring;
