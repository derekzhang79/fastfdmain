    ...
    ui(new Ui_w_sys_managepromotion_setting)
{
    ...
    tablemodel_dish->save_set_header("ch_dishno", tr("菜品编码"));
    tablemodel_dish->save_set_header("vch_dishname", tr("菜品名称"));
    tablemodel_dish->save_set_header("num_price", tr("原价"), true);
    tablemodel_dish->save_set_header("num_special_price", tr("特价"), true);
    tablemodel_dish->save_set_header("num_m_price", tr("会员价"), true);
    tablemodel_dish->save_set_header("ch_special_type", tr("策略")+"1", true);
    tablemodel_dish->save_set_header("ch_special_type2", tr("第二份半价"), true);
    tablemodel_dish->tablename_kvmap_insert("ch_special_type", "0", tr("默认原价"));
    tablemodel_dish->tablename_kvmap_insert("ch_special_type", "1", tr("默认特价"));
    tablemodel_dish->tablename_kvmap_insert("ch_special_type", "2", tr("可选特价"));
    tablemodel_dish->tablename_kvmap_insert("ch_special_type2", "0", tr("不启用"));
    tablemodel_dish->tablename_kvmap_insert("ch_special_type2", "1", tr("启用"));
    ui->promotion_setting_target->addItem(tr("所有人"), 0);
    ui->promotion_setting_target->addItem(tr("会员"), 1);
    ui->promotion_setting_target->addItem(tr("非会员"), 2);
    ...
    scan_all_objectname_add_map_setFilter("vch_parameter" ,"promotion_setting");
...
{
    lds_widget_auto_enable d(ui->pushButton_ok);
    saveData();
}
...
bool w_sys_manage_promotion_setting::saveData()
{
    ...
    if(tablemodel_dish->trySubmitAll() //每个菜品的设置
            && mapper_save_data()) {
        lds_query::com_mit();
