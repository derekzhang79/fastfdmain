...
void w_sys_manage_wirelessordermonitoring::initdata(QWidget *parent, const QString &dir)
{
    ...
    //!短信息编码
    if(0 == lds_messagebox::question(parent, MESSAGE_TITLE_VOID, tr("初始化基础资料成功")+"\n"+info)){
...
bool w_sys_manage_wirelessordermonitoring::startjz(QWidget *parent)
{
    ...
    public_sql::genrateShPid(wxpidlist, "pos/w", "pos/wx");
    if(wxpidlist.count() == 0){
        QProcess::startDetached(qApp->applicationDirPath()+"/wx" ,
                                QStringList() ,
...
send:
    ...
    writeback(printer_0::trans_u2g(senddata), baseNo);
    ...
    if(w_scr_dish_restaurant_div_pop::getMaxChbillnoAdd1_insert_cey_u_master("B", ch_billno)) {
        if(query.execUpdate("cey_bt_table",  qrtVariantPairList()
            ...
            if(query.execInsert("cey_u_table", qrtVariantPairList()
                                ...
                                << qrtVariantPair("vch_operID", dcj_opid_map.value(dcjno_3))
...
0105    01002    1                 0 00:02:26      2 */
    ...
    foreach(const QString &p, infolist){
        ...
        if(w_scr_dish_restaurant_div_pop::cey_bt_table_has_locked(tableno_4, 0)) {
            goto rollback;
        }
...
interloopp0:
        ...
        int_rate = query.recordValue("int_rate").toString() ;

        float num_price_add =  tablemodel.get_num_price_add_vch_print_memo_by_memo(vch_cook);
...
*/
    ...
    query.execSelect(QString("select sum(num_num - num_back) from cey_u_orderdish where ch_billno = '%1' and ch_dishno = '%2' ")
                     ...
                     .arg(dishno));
    query.next();
    info = tr("退菜数量超过总量%1(%2)").arg(dishnum_back_total).arg(query.recordValue(0).toFloat());
...
void w_sys_manage_wirelessordermonitoring_box::THKX(int baseNo)
{
    ...
    query.execSelect(QString("select ch_tableno, vch_tablename, ch_billno  from cey_bt_table   where  ch_tableno = '%1'  ")
                     .arg(tableno_4));
    if(query.next()) {
        info = query.recordValue("vch_tablename").toString()+tr("开台状态:")+"\n";
        ch_billno = query.recordValue("ch_billno").toString();
        if(ch_billno.isEmpty()){
            info += tr("未开台");
        } else {
...
bool w_sys_manage_wirelessordermonitoring_box::quitdish(QString &errstring, const QString &ch_billno, const QString &ch_dishno, double dishnum_back_total, const QString &vch_reason, const QString &operid)
{
    lds_query query;
