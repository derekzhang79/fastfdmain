...
//    void toTC(const QByteArray &request);//退菜
...
//    void toZZCC(const QByteArray &request);//  整桌催菜
//    void toCLCC(const QByteArray &request);//  菜品类别催菜
//    void toCPCC(const QByteArray &request);//  菜品催菜
...
//    //整桌催菜(ZZCC)、按菜品催菜(CPCC)

//private:
//    bool tocheckoperidRight(lds_query &query,const QString &dcjno_3, const QString &rightno = "");
//public slots:
...
public:
    ...
    void ZZCC(int baseNo);//  整桌催菜
    void CLCC(int baseNo);//  菜品类别催菜
    void CPCC(int baseNo);//  菜品催菜
public:
    ...
    bool quitdish(QString &errstring, const QString &ch_billno, const QString &ch_dishno, double dishnum_back_total, const QString &vch_reason, const QString &operid);
protected:
