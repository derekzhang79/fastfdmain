    ...
    ui(new Ui::getFilePath)
{
    ...
    path_arg  =
            ...
            "<span style=\" color:#ff650b;font-weight: bold;\">%3</span>";
    ...
    //
    QRadioButton *r;
    radio_str_pair_list.append(radio_strPair(r = new QRadioButton(tr("系统目录"), this), "/usr/desk")); r->setStyleSheet("QRadioButton:indicator{ height:20px;width:20px; image: url(:/image/q6.png);} ");r->setFixedHeight(30);
    radio_str_pair_list.append(radio_strPair(r = new QRadioButton(tr("SD卡"), this), "/mnt/sdcard"));r->setStyleSheet("QRadioButton:indicator{ height:20px;width:20px; image: url(:/image/q7.png);} ");r->setFixedHeight(30);
...
void getFilePath::on_pushButton_copy_pressed()
{
    ...
    if(copychecked){
        ...
        ui->tableWidget_dir->getGuideLabel()->setText(path_arg.arg(tr("复制")+filepathfrom).arg(tr("到")).arg(pathto+QDir::separator()));
    }
}
...
void getFilePath::on_pushButton_paste_clicked()
{
    ...
    if(QFile::exists(filepathto)){
        if(0 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("删除同名文件?"), tr("是"),tr("否"))){
...
failure:
    lds_messagebox::warning(this, MESSAGE_TITLE_VOID, error+tr("操作失败"));
ret:
    copy_pasteupdate();
    return;
}
...
void getFilePath::on_pushButton_del_clicked()
{
    ...
    if(filepath.isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("没有任何文件被选中"));
        return;
    }
...
void getFilePath::toupdateGuide(QString path,QString name)
{
    ...
    if(filepathfrom.isEmpty() && pathto.isEmpty()){
        ...
        pathto = filepath;
        ui->tableWidget_dir->getGuideLabel()->setText(path_arg.arg(filepathfrom).arg(tr("到")).arg(pathto+QDir::separator()));
    }
}
...
bool getFilePath::deleteDir(const QString &path)
{
    ...
    if(path.isEmpty()) return false; //当treeview没有选中行时
...
bool getFilePath::copyFile(const QString &fromfilepath, const QString &topath)
{
    ...
    if(!flag) qDebug() << file0.errorString();
    return flag;
}
...
void getFilePath::on_pushButton_rename_clicked()
{
    ...
    if(QDialog::Accepted == lds_roundeddialog_rect_align(&dialog).exec()){
        if(dialog.textValue() == QFileInfo(filepath).fileName()){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("名字没有改变"));
            return;
        }
...
void getFilePath::updatecopy_paste(bool f)
{
    ...
    if(copychecked)ui->pushButton_copy->setText(tr("取消复制"));
    else ui->pushButton_copy->setText(tr("复制"));
...
QString getFilePath::getOpenFileName(QWidget *parent, const QString &caption, const QString &dir, const QString &filter, QString *selectedFilter, QFileDialog::Options options)
{
    ...
    if(QDialog::Accepted == lds_roundeddialog_rect_align(&dialog).exec()){
        ...
        if(QFileInfo(dialog.retpath).isDir()){
            lds_messagebox::warning(parent, MESSAGE_TITLE_VOID, tr("非文件")+ "," + tr("操作失败"));
            return "";
        }
...
QString getFilePath::getSaveFileName(QWidget *parent, const QString &caption, const QString &dir, const QString &filter, QString *selectedFilter, QFileDialog::Options options)
{
    ...
    if(QDialog::Accepted == lds_roundeddialog_rect_align(&dialog).exec()){
        if(QFile::exists(dialog.retpath)){
            if(1 == lds_messagebox::warning(parent, caption,tr("有同名文件,该文件将会被替换"), tr("确定"), tr("取消"))){
                return "";
            }
        }
        if(!QPixmap(dialog.retpath).isNull()){
