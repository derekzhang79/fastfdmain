...
void QColorPicker::paintEvent(QPaintEvent* )
{
    ...
    //    p.setPen(Qt::white);
    p.fillRect(0, pt.y(), r.width(),  1, QColor("#ff650b"));
    p.fillRect(pt.x(), 0, 1, r.height(), QColor("#ff650b"));
    p.fillRect(pt.x()-2, pt.y()-2, 5, 5, QColor("#ff650b"));
    /*
...
//    p.setPen(g.foreground().color());
    p.setPen(Qt::NoPen);
    p.setBrush(QColor("#ff650b"));//g.foreground());
    QPolygon a;
    int y = val2y(val);
    a.setPoints(3, w, y, w+5, y+5, w+5, y-5);
//    p.eraseRect(w, 0, 5, height());
    p.drawPolygon(a);
    p.fillRect(0, y, wi, 1, QColor("#ff650b"));
}
