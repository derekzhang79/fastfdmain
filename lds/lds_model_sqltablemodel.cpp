...
lds_model_sqltablemodel::lds_model_sqltablemodel(QObject *parent)
    : QSqlTableModel(parent, lds_thread::getThreadDbAndOpen())
{
    select_count = 0;
}
...
{
    if(!setFilter_select)  setFilter_select = true;

    select_count ++;
    qDebug() << "SELECT_COUNT:" << this << select_count;
    bool f = QSqlTableModel::select();
    //清除缓存
    tablename_kvmap_clear_for_select_and_autoselect();
...
{
    insert_row_where_sql_map(index.row());
...
void lds_model_sqltablemodel::SELECT_AUTO_REFRESH_refresh(const QString &tablename) const
{
    ...
    if(0 == d) {
        tablename_kvmap_insert(tablename);
...
void lds_model_sqltablemodel::tablename_kvmap_insert(const QString &tablename) const
{
    //1
    lds_model_sqltablemodel_d *d = tablename_kvmap.value(tablename, 0);
    if(0 == d) {
...
void lds_model_sqltablemodel::tablename_kvmap_insert(const QString &tablename, const QString &key, const QVariant &value) const
{
    //1
    ...
    tablename_kvmap_insert(tablename);
    //3
    tablename_kvmap.value(tablename)->setData(key, "", value);//->insert(key, value);
}

void lds_model_sqltablemodel::tablename_kvmap_insert(const QString &tablename, const QString &sql_keyname, const QString &sql_valuename, const QString &sql_filter, bool firstEmpty, const QString &SELECT_AUTO_REFRESH_KEY)
{
    if(this->SELECT_AUTO_REFRESH_KEY == SELECT_AUTO_REFRESH_KEY) {
        if(SELECT_AUTO_REFRESH_map.find(tablename) == SELECT_AUTO_REFRESH_map.end()) {
            SELECT_AUTO_REFRESH_map.insert(tablename, SELECT_AUTO_REFRESH_data(sql_keyname , sql_valuename , sql_filter , firstEmpty));
        }
        SELECT_AUTO_REFRESH_refresh(tablename);
    }
}

void lds_model_sqltablemodel::tablename_kvmap_insert(const QString &tablename, const QString &key_value_sql, bool firstEmpty, const QString &SELECT_AUTO_REFRESH_KEY)
{
    if(this->SELECT_AUTO_REFRESH_KEY == SELECT_AUTO_REFRESH_KEY) {
        if(SELECT_AUTO_REFRESH_map.find(tablename) == SELECT_AUTO_REFRESH_map.end()) {
...
QVariant lds_model_sqltablemodel::sql_value(const QString &tablename, const QString &key, const QString &sql_keyname, const QString &sql_valuename, const QString &sql_filter) const
{
    //拼接filter
    QString filter;
    filter = sql_keyname + "= '" +key+"'";
...
void lds_model_sqltablemodel::tablename_kvmap_clear(const QString &tablename) const
{
...
void lds_model_sqltablemodel::tablename_kvmap_clear_for_select_and_autoselect()
{
    QMapIterator< QString/*tablename*/, lds_model_sqltablemodel_d *>   map_itr(tablename_kvmap);
    while(map_itr.hasNext()) {
        map_itr.next();
        if(SELECT_NOT_CLEAR_MAP_KEY_set.contains(map_itr.key())) continue;
        tablename_kvmap_clear(map_itr.key());
    }
...
QVariant lds_model_sqltablemodel::tablename_kvmap_value(const QString &tablename, const QString &key, const QString &sql_keyname, const QString &sql_valuename, const QString &sql_filter, const QVariant &default_value) const
{
    ...
    if(!d->isFill(key, sql_valuename)) {//列表中不存在
        d->setData(key, sql_valuename, sql_value(tablename, key, sql_keyname, sql_valuename, sql_filter));
    }

    if(!d->isValid(key, sql_valuename) && default_value.isValid()) {//存在但为null
        d->setData(key, sql_valuename, default_value);
    }
    return d->data(key, sql_valuename);
}

QVariant lds_model_sqltablemodel::tablename_kvmap_value_direct(const QString &tablename, const QString &key) const
{
    QMap<QString/*tablename*/, lds_model_sqltablemodel_d *>::const_iterator k = tablename_kvmap.find(tablename);
    if(k != tablename_kvmap.end()){
        return k.value()->data(key);
    }
    return QVariant();
}
...
void lds_model_sqltablemodel::insert_row_where_sql_map(int row)
{
    ...
    QString sql_del_where_sql;
    for(int p = 0; p < primaryKey().count(); p ++) {
        QString kname = primaryKey().fieldName(p);
        QString kvalue = model_data(row, kname).toString();
        sql_del_where_sql += qrtEqual(kname, kvalue) + " and ";
    }
    sql_del_where_sql.chop(5);
    row_where_sql_map.insert(row, sql_del_where_sql);
}
