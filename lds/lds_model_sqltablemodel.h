...
protected:
    ...
    void tablename_kvmap_insert(const QString &tablename) const;//直接插入
public:
    //插入获取map(tablename, 的map(keyname, valuename))
    ///插入固定的key和value
    void tablename_kvmap_insert(const QString &tablename
                                , const QString &key
                                , const QVariant &value) const;//直接插入
    //返回map<key, value>， 若map为空则更新数据
    ///插入sql的检索条件，select时，会自动刷新，要求，sql的检索结果要尽可能的少
    void tablename_kvmap_insert(const QString &tablename
                                , const QString &sql_keyname, const QString &sql_valuename, const QString &sql_filter
                                , bool firstEmpty
...
public:
    //清除map<tablename, map>
    void tablename_kvmap_clear_for_select_and_autoselect();
    void tablename_kvmap_clear(const QString &tablename) const;
private:
    void tablename_kvmap_clear_all();

public:
    QVariant tablename_kvmap_value_direct(const QString &tablename,
...
private:
    QVariant sql_value(const QString &tablename, const QString &key,
