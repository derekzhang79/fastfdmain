...
{
    QString dbname = lds_thread::getThreadDbname();
...
QDateTime lds_query_db::curdate()
{
    QDateTime dt;
    //MYSQL
...
#ifdef QT_DEBUG
    //    if(!ret)  qDebug()  << "cur thread id:" << QThread::currentThreadId() << "main thread id:" << lds_thread::MAIN_THREAD_ID<<  query << this->lastError();
    //    else
    //        qDebug() << query;
#endif
...
#ifdef QT_DEBUG
    //    if(!ret)  qDebug()  << "cur thread id:" << QThread::currentThreadId() << "main thread id:" << lds_thread::MAIN_THREAD_ID<<  this->lastQuery() << this->lastError();
#endif
    return ret;
}
...
bool lds_query_db::execUpdate(const QString &table, const qrtVariantPairList &key_value_list, const QString &where_sql, QString *errstring)
{
...
bool lds_query_db::execInsert(const QString &table, const qrtVariantPairList &key_value_list, QString *errstring)
{
    //prepare_str
    //into
    QString prepare_str;
...
{
    *(qrtVariantType *)&_type = type;
}

QString qrtVariant::toString() const
{
    QVariant::Type datatype = _value.type();
    if(QVariant::Date == datatype) {
        return _value.toDate().toString("yyyy-MM-dd");
    }
    if(QVariant::Time == datatype) {
        return _value.toTime().toString("hh:mm:ss");
    }
    if(QVariant::DateTime == datatype) {
        return _value.toDateTime().toString("yyyy-MM-dd hh:mm:ss");
    }
    if(QVariant::Double == datatype) {
        return QString("%1").arg(_value.toDouble());
    }
    return _value.toString();
}


void lds_query_db::execUpdateTrigger(const QString &table, const qrtVariantPairList &key_value_list, const QString &where_sql)
{
    //ch_yun_already_keep 若是主动更新该字段，则不进行update
    bool ch_yun_already_keep_update = db.record(table).field("ch_yun_already_keep").isValid();
    if(ch_yun_already_keep_update) {
        for(int k = 0; k < key_value_list.count(); k++) {
            if(key_value_list[k].first == "ch_yun_already_keep")  {
                ch_yun_already_keep_update = false;
                break;
            }
        }
        if(ch_yun_already_keep_update)
            *(qrtVariantPairList *)&key_value_list << qrtVariantPair("ch_yun_already_keep", "N");
    }
}

void lds_query_db::execInsertTrigger(const QString &table, const qrtVariantPairList &key_value_list)
{
    ...
    if(ch_yun_already_keep_update) {
        for(int k = 0; k < key_value_list.count(); k++) {
            if(key_value_list[k].first == "ch_yun_already_keep")  {
                key_value_list[k].second.setType(qrtVariant::NONE);
                break;
            }
        }
        if(ch_yun_already_keep_update)
    ...
    if(db.record(table).field("terminal_code").isValid()) {
        ...
        *(qrtVariantPairList *)&key_value_list << qrtVariantPair("terminal_code", lds::terminalCode);
    }
}
