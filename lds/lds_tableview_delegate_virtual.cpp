...
void lds_tableview_delegate_virtual::delay_tableview_resize_column_to_content(lds_tableView *tableview, int column)
{
    ...
    for(int k = 0; k < count; k++) {
        if(k >= tableview->verticalHeader()->count()) break;
        if(tableview->columnSpan(row + k, column) != 1) break;
    ...
    QStyledItemDelegate(parent)
{
    ...
    field_name_mode = 0;
    bp_fmt_price_dec = lds_query::selectValue(query, " select vch_value from cey_sys_parameter where vch_parameter = 'bp_fmt_price_dec' ").toInt();
    bp_fmt_num_dec = lds_query::selectValue(query, " select vch_value from cey_sys_parameter where vch_parameter = 'bp_fmt_num_dec' ").toInt();
    bp_fmt_amount_dec = lds_query::selectValue(query, " select vch_value from cey_sys_parameter where vch_parameter = 'bp_fmt_amount_dec' ").toInt();
}
...
void lds_tableview_delegate_virtual::paintText(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index,  const QVariant &dd) const
{
    ...
    if(lds_tableView::model_data_is_num(dd)) {
    ...
    //
    if(1 == field_name_mode) {//奇数是num_dish, 偶数是double
        if(lds_tableView::model_data_is_num(dd)) {
...
int lds_tableview_delegate_virtual::getType()
{
    return -1;
}
...
QString lds_tableview_delegate_virtual::paint_transfer(const QVariant &dd, const QString &field_name) const
{
    ...
    if(dd.type() == QVariant::Double) {
        return lds::septNumber(dd.toDouble(), field_name_decimal(field_name));
    }
    switch(field_name_mode) {
    case 0://正常
        break;
    case 1://奇数是num_dish, 偶数是double
        return w_rpt_report_frame::headdata_transfer(dd);//
    }
...
int lds_tableview_delegate_virtual::field_name_decimal(const QString &field_name) const
{
    //数量
...
void lds_tableview_delegate_virtual::enableFieldName2k0v1()
{
    field_name_mode = 1;
}
