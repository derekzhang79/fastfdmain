...
bool Reader_Tel::open(QString *errstring){
    ...
    if((Curdev = libusb_open_device_with_vid_pid(ctx,0x483,0x5750)) == NULL){//打开失败则 退出 ->Curdev == NULL
        isopen = false;
    ...
    //5声明
    if((r=libusb_claim_interface(Curdev,0)) < 0){//请求接口（需要你知道设备的接口数值Curdev） 如果请求失败
        isopen = false;
...
info_tel Reader_Tel::getTelInfo()
{
    ...
    int actual = -1; // 实际读到的
    mutex.lock();
    libusb_bulk_transfer(Curdev,(0x81),(uchar*)byte2.data(),byte2.length(),&actual,1000);
    mutex.unlock();
    if(actual == 0){
