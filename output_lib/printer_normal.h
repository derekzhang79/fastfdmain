...
public:
    Printer_Normal();
    virtual ~Printer_Normal();
    virtual blockIns tran(const blockIns &bins, int xinye_beep) = 0;
    ...
    static Printer_Normal *byName(const QString &fname, QString &return_typename);
...
//    resolve::Enhance, Printer_Normal_58,32,ins
    static void resolve(int print_instype, QString print_class_name, int paperwidth, int xinye_beep_count, blockIns &ins);
    static const QMap<QString/*print_class_name, name*/, QVariant> &get_normal_print_map_ref();
    static const QMap<int/*Normal_INS_TYPE*/, QString/*name*/> &get_normal_ins_map_ref();
    //
    //解析后，打印，关闭接口
    static bool toprint(int print_instype, QString print_class_name, int paperwidth, int xinye_beep_count,
                        const QString &path, blockIns ins, int times = 1, QString *errstring = 0);
    //打印，关闭接口
    static bool toprint(const QString &path, blockIns ins, int times = 1, QString *errstring = 0);
    //打印，不关闭接口
    static bool toprint(Printer_POS *printer, const blockIns &ins, int times = 1);
private:
    //
};
//注册
