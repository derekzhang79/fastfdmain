...
//有效传输密码和用户密码为6字节， 但通讯按8 字节发送， 最后两个字节为任意值。
...
bool Reader_RF::open()
{
    ...
    //按vid,pid打开
    if((Curdev = libusb_open_device_with_vid_pid(ctx,0x10C4,0x82CD)) == NULL){//打开失败则 退出 Curdev == NULL
        isopen = false; //该函数 包含了libusb_get_device_list的判断
        return isopen;
    }
    //声明前判断
    ...
    read_card_error=
            QStringList()
            <<""<<tr("在操作区域内无卡")<<""<<""
           <<tr("验证不成功")<<""<<tr("通讯错误")<<""<<tr("读系列号错误")<<""<<tr("卡片没有通过验证");
    //默认密码
...
bool Reader_RF::writecard(const QString &part0, const QString &part1, const QString &part2)
{
    ...
    if((r = getRespone(Curdev,tmp45)) < 0 ){
    ...
    if((r = getRespone(Curdev,tmp41)) < 0 ){
        otherError = libusb_error_name(r);
        return false;
    }
    ...
    if((r = getRespone(Curdev, tmp42)) < 0 ){
        otherError = libusb_error_name(r);
        return false;
    }
    ...
    if((r = getRespone(Curdev, tmp43)) < 0 ){
        otherError = libusb_error_name(r);
        return false;
    }
    ...
    if((r = getRespone(Curdev, tmp5f)) < 0 ){
        otherError = libusb_error_name(r);
        return false;
    }
    ...
    //    if((r = getRespone(Curdev, tmp4c)) < 0 ){
    //        otherError = libusb_error_name(r);
    //        return false;
    //    }
    ...
    //    if((r = getRespone(Curdev, tmp44)) < 0 ){
    //        otherError = libusb_error_name(r);
    //        return false;
    //    }
    if(indexof_simpleError() != 0 ){
        otherError = /*simpleError()+*/tr("密码错误");
        return false;
    }
...
bool Reader_RF::readcard(QString &part0, QString &part1, QString &part2)
{
...

    {
        ...
        if((r = getRespone(Curdev, tmp46)) < 0 ){
            //() << "getRespone(Curdev,tmp46) = " << libusb_error_name(r);
            otherError = libusb_error_name(r);
...
bool Reader_RF::pwdChange(const QString &oldPwd, const QString &newPwd)
{
    ...
    if((r = getRespone(Curdev, tmp4e)) < 0 ){
        otherError = libusb_error_name(r);
        return false;
    }
    ...
    if((r = getRespone(Curdev,tm47_3)) < 0 ){
        otherError = libusb_error_name(r);
        return false;
    }
    if(indexof_simpleError() != 0)  return false;
...
bool Reader_RF::beep()
{
    ...
    int r=libusb_control_transfer(Curdev,
