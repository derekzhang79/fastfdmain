...
{
    ui->setupUi(this);
    setWindowFlags(Qt::Dialog|Qt::FramelessWindowHint);this->setLayoutDirection(CommonHeader::com_widget_dir);
    {
        if(QSqlDatabase::connectionNames().contains("barscale_main_dialog")){
            ...
            db.setDatabaseName(CommonHeader::com_root_dirpath+"/db_barscale_main_dialog.db");
        }
    }
    ...
    ui->spinBox_precision_amount->setValue(conf.value("barscale/precision_amount", 2).toInt());
    ui->spinBox_precision_weight->setValue(conf.value("barscale/precision_weight", 3).toInt());
    ui->spinBox_precision_price->setValue(conf.value("barscale/precision_price", 2).toInt());
    ui->comboBox_unitno->setCurrentIndex(conf.value("barscale/scaleunitno").toInt());
...
void barscale_main_dialog::keep(QSqlQuery &queryouter, const QString sqlwherebarcode)
{
    ...
    while(query.next()){
        ...
        if(queryouter.next()){//存在，同步
            if(!querygo.exec(QString("update barscale_main_dialog_table set vch_name='%1', vch_price='%2' , vch_validdate = '%4' where vch_no='%3' ")
...
void barscale_main_dialog::update_downmodel()
{
    ...
    for(int row=0,rowcount=downmodel0->rowCount();row<rowcount;row++){
        ...
        creatItem(items, QStringList()
                  ...
                  << QString::number((downmodel0->record(row).value("vch_price").toFloat() * 100))
...
bool barscale_main_dialog::brefore_getscaletypes()
{
    ...
    dialog.setWindowTitle(tr("选择条码秤"));
...
void barscale_main_dialog::on_pushButton_5_clicked()
{
        ...
        for(row = 0,rowcount = downmodel0->rowCount(); row < rowcount ;row++){
            for(rowsub = row+1; rowsub < rowcount; rowsub++){
                if(downmodel0->data(downmodel0->index(row, 0)).toString()
                    ...
                    if(0 == uinormal_message::warning(this, tr("提示"),  tr("plu编码重复:%1行和%2行重复").arg(row+1).arg(rowsub+1), tr("返回修改"),tr("继续退出"))){
                        return;
                    }
                    goto end;
                }
    ...
    if(!downmodel0->submitAll()){
        if(0 == uinormal_message::warning(this, tr("提示"), downmodel0->lastError().text(),tr("强制修改"), tr("继续退出"))){
            QSqlDatabase::database("barscale_main_dialog").transaction();
            querygo.exec("delete from barscale_main_dialog_table");
            for(int row = 0, rowcount = downmodel0->rowCount(); row < rowcount ;row ++){
                querygo.prepare("insert into barscale_main_dialog_table(vch_plu, vch_name, vch_no,vch_price,  vch_weighttype, vch_validdate,vch_flag  ) values(?,?,?,?,?, ?,?)");
                ...
                QSqlDatabase::database("barscale_main_dialog").rollback();
                if(0 == uinormal_message::warning(this, tr("提示"),querygo.lastError().text(),tr("返回修改"), tr("继续退出"))){
                    return;
                }
                goto end;
            }
            QSqlDatabase::database("barscale_main_dialog").commit();
            uinormal_message::warning(this, tr("提示"),tr("保存成功"));
        }
    }
    //4
end:
...
void barscale_main_dialog::update_barcodetype(const QString scaletype)
{
    ...
    case 3://厦门顶尖LS2CX
    {
        if(conf.value("barscale/djianpisnotop", false).toBool()==false){
            QDialog dialog(this);
    ...
    case 5://容大RLS1000
        if(this->sender() != 0) {
            uinormal_message::information(this, tr("提示"), tr("容大条码秤PLU号为行号,最多252个\nPLU号同商品代码一一对应"));
        }
...
void barscale_main_dialog::save_barscale()
{
    ...
    conf.setValue("barscale/precision_amount",ui->spinBox_precision_amount->value());
    conf.setValue("barscale/precision_weight",ui->spinBox_precision_weight->value());
    conf.setValue("barscale/precision_price",ui->spinBox_precision_price->value());
    conf.setValue("barscale/scaleunitno",ui->comboBox_unitno->currentIndex());
}
...
QWidget *barscale_main_delegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if(c == 4){
        ...
        editor->addItems(CommonHeader::creat_barcode_scale_weightype(QSettings(CommonHeader::com_ini_filepath, QSettings::IniFormat).value("barscale/scaletype").toString()).values());
        return editor;
    } else {
...
void barscale_main_delegate::setEditorData(QWidget *editor, const QModelIndex &index) const
{
    ...
    if(c == 4){
        ...
        spinBox->setCurrentIndex(spinBox->findText(CommonHeader::creat_barcode_scale_weightype(QSettings(CommonHeader::com_ini_filepath, QSettings::IniFormat).value("barscale/scaletype").toString())[value.toInt()]));
    } else {
...
void barscale_main_delegate::setModelData(QWidget *editor, QAbstractItemModel *model, const QModelIndex &index) const
{
    ...
    if(c == 4){
        QComboBox *spinBox = static_cast<QComboBox*>(editor);
        model->setData(index, CommonHeader::creat_barcode_scale_weightype(QSettings(CommonHeader::com_ini_filepath, QSettings::IniFormat).value("barscale/scaletype").toString()).key(spinBox->currentText()), Qt::EditRole);
    } else {
...
void barscale_main_delegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if(c != 4){
        ...
        painter->drawText(x, flag, CommonHeader::creat_barcode_scale_weightype(QSettings(CommonHeader::com_ini_filepath, QSettings::IniFormat).value("barscale/scaletype").toString())[key]);//);
    }
}
...
void barscale_main_dialog::on_pushButton_export_clicked()
{
    ...
    //获取路径
    QString filepath = QFileDialog::getSaveFileName(this, tr("导出文件"),CommonHeader::com_root_dirpath+QDir::separator()+"barscale_main.csv");
    if(filepath.isEmpty()){
        return;
    }
    QFile file(filepath);
...
{
    //获取路径
    QString filepath = QFileDialog::getOpenFileName(this, tr("导入文件"),CommonHeader::com_root_dirpath);
...
void barscale_main_dialog::on_pushButton_allcontrol_clicked()
{
    ...
    if(indexlist.isEmpty()){
        uinormal_message::warning(this, tr("批量修改"), tr("没有数据被选中"));
        return;
    }
    barscale_num_input inputdialog(this);
    inputdialog.setWindowTitle(tr("批量修改"));
...
void barscale_main_dialog::on_pushButton_switch_mode_clicked()
{
    ui->tableView->selectionModel()->clear();
    if(switch_mode == 0){
        ui->pushButton_switch_mode->setText(tr("批处理模式(点击切换)"));
        switch_mode = 1;
        ...
        if(switch_mode == 1){
            ui->pushButton_switch_mode->setText(tr("正常模式(点击切换)"));
            switch_mode = 0;
