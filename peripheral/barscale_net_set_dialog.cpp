    ...
    _parent(parent)
{
    ...
    this->setWindowTitle(tr("网络设置-新增"));

    model = new QStandardItemModel;
    model->setHorizontalHeaderLabels(QStringList() << tr("秤号") << tr("秤名") << tr("地址/端口") << tr("状态"));
...
void barscale_net_set_dialog::on_pushButton_clicked()
{
    ...
    if(QDialog::Accepted==dialog.exec()){
        model->appendRow(QList<QStandardItem*>()
                         ...
                         << new QStandardItem(dialog.ui->checkBox_isuse->isChecked()?tr("使用"):tr("不使用"))
                         );
    }
}
...
void barscale_net_set_dialog::on_pushButton_2_clicked()
{
    ...
    if((row=ui->tableView->currentIndex().row()) >= 0){
        ...
        dialog.ui->checkBox_isuse->setChecked(model->item(row, 3)->text()==tr("使用"));
        ...
        if(QDialog::Accepted==dialog.exec()){
            ...
            model->item(row, 3)->setText(dialog.ui->checkBox_isuse->isChecked()?tr("使用"):tr("不使用"));
        }
    }
}
...
void barscale_net_set_dialog::save_setting()
{
    ...
    conf.setValue("barscale/netinfo", ba);
}
...
{
    ui->setupUi(this);setWindowFlags(Qt::Dialog|Qt::FramelessWindowHint);this->setLayoutDirection(CommonHeader::com_widget_dir);
    this->setWindowTitle(tr("网络设置-修改"));
...
{
    ui->comboBox_name->setCurrentIndex(ui->comboBox_name->findText(QSettings(CommonHeader::com_ini_filepath, QSettings::IniFormat).value("barscale/scaletype").toString()));
