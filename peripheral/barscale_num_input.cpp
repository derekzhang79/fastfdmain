    ...
    ui(new Ui::barscale_num_input)
{
    ...
    updatem(conf.value("barscale_num_input/mode", 0).toInt());
...
void barscale_num_input::updatem(int m)
{
    ...
    switch(m){
    case 0:
        ui->pushButton_switch->setText(tr("替换"));
        break;
    case 1:
        ui->pushButton_switch->setText(tr("倍数"));
        break;
    }
    conf.setValue("barscale_num_input/mode", m);
    mode = m;
}
