...
QHash<int, QString> CommonHeader::baudrates;
...
QHash<int, QString> CommonHeader::creat_baudrates()
{
    if(baudrates.isEmpty()){
        baudrates = Printer_POS_SERIAL::creat_baudrates();
    }
    return baudrates;
}

Printer_POS *CommonHeader::getposprint_bypath(const QString &path)
{
    return Printer_POS::get_pos_printer_by_path(path);
}
...
QStringList CommonHeader::creat_pricecount_scaletypes()
{
    if(pricecout_scaletypes.isEmpty()){
        pricecout_scaletypes = ElectronicCountPrice::creat_pricecount_scaletypes();
        ;
    }
...
QStringList CommonHeader::creat_normal_printpathtypes()
{
    if(normal_printpathtypes.isEmpty()){
        normal_printpathtypes << QObject::tr("USB") << QObject::tr("串口") << QObject::tr("网口") << QObject::tr("并口");
    }
...
bool CommonHeader::barcode_scale_resolve(QString &barcode, QString &price, QString &weight, QString &amount, const QString &code)
{
    ...
    int precision_amount = conf.value("barscale/precision_amount", 2).toInt();
    int precision_weight = conf.value("barscale/precision_weight", 3).toInt();
    int precision_price = conf.value("barscale/precision_price", 2).toInt();
