...
public:
    ...
    static Printer_POS *getposprint_bypath(const QString &path);//注意删除该指针
    ...
    static bool barcode_scale_resolve(//解析 条码秤一段条码
            ...
            QString &amount,
            const QString &code);
    static QString com_ini_filepath;//记录外设一些通用的配置文件路径
    static QString com_root_dirpath;//记录外设导出文本的一根目录，必须保证路径存在
    static Qt::LayoutDirection com_widget_dir;//记录外设导出文本的一根目录，必须保证路径存在
    static QString get_barcode_scaleflag();
    static QString barcode_scaleflag;
private:
    //normal_print
    ...
    static QHash<int, QString> baudrates;
