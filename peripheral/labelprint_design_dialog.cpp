...
{
    QString errstring;
    if(!labelprint_main_dialog_object::savePrintMap(CommonHeader::com_root_dirpath+"/def.txt", design->printMap(), &errstring)) {
        uinormal_message::warning(this, tr("提示"),  errstring);
        return;
    }
}
...
void labelprint_design_dialog::toload()
{
    SSRPairMap ssrmap;
    QString errstring;
    if(!labelprint_main_dialog_object::getPrintMap(CommonHeader::com_root_dirpath+"/def.txt", ssrmap, &errstring)) {
        uinormal_message::warning(this, tr("提示"),  errstring);
        return;
    }
...
void labelprint_design_dialog::designprint(QWidget *parent, bool istest, bool pop)
{
    ...
    if(!print_help.printLabel(design->printmodel, design->printMap(), istest, &errstring)) {
        if(pop)uinormal_message::warning(parent, tr("标价打印机:"), errstring);
        return;
    }
    if(pop)uinormal_message::warning(parent, tr("标价打印机:"),tr("打印成功"));
...
void labelprint_design_dialog::todefbill()
{
    ...
    restore_ssrmap.insert(tr("品名"),QVariantList() << (int)Shape::Rectangle << QRect(600,300,200,30) << QString("1"));//
    restore_ssrmap.insert(tr("条码"),QVariantList() << (int)Shape::OneBarCode << QRect(600,250,200,30) << QString("1"));
    restore_ssrmap.insert(tr("价格"),QVariantList() << (int)Shape::Rectangle << QRect(600,200,200,30) << QString("1"));
...
void labelprint_design_dialog::toimportbill()
{
    ...
    if(dialog.getfilename().isEmpty()){
        uinormal_message::warning(this, tr("标价打印机:"), tr("没有找到文件"));
        return;
    }
    QString errstring;
    SSRPairMap ssrmap;
    if(!labelprint_main_dialog_object::getPrintMap(dialog.getfilename(), ssrmap, &errstring)) {
        uinormal_message::warning(this, tr("标价打印机:"), errstring);
        return;
    }
    design->initDocument(ssrmap);
}
...
void labelprint_design_dialog::tosavebill()
{
    ...
    dialog.setLayoutDirection(CommonHeader::com_widget_dir);
    //! ========================
    if(QDialog::Accepted!=dialog.exec()) return;

    if(sui.lineEdit_filename->text().isEmpty()){
        uinormal_message::warning(this, tr("标价打印机:"), tr("标价打印机错误:")+tr("文件名为空"));
        return;
    }
    ...
    if( (filenams.contains(sui.lineEdit_filename->text()) && uinormal_message::warning(this, tr("标价打印机:"), tr("标价打印机:")+tr("有同名文件,是否覆盖?"), tr("确定"), tr("取消"))==0)
        ...
        uinormal_message::warning(this, tr("标价打印机:"), tr("保存成功"));
    }
}
