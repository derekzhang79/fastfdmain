...
{
    ui->setupUi(this);
    setWindowTitle(tr("表单导入"));
    setWindowFlags(Qt::Dialog|Qt::FramelessWindowHint);
    setLayoutDirection(CommonHeader::com_widget_dir);
...
{
    if(1==uinormal_message::warning(this, tr("表单导入"),tr("是否确定删除?"),tr("确定"),tr("取消"))) return;
    QFile f(_dir+"/"+ui->comboBox->currentText());
    if(f.remove()){
        updatecom();
    } else {
        uinormal_message::warning(this, tr("表单导入"),f.errorString());
    }
}
