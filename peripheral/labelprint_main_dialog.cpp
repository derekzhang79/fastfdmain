...
void labelprint_main_dialog::inputfinished(bool label)
{
    ...
    db.setDatabaseName(CommonHeader::com_root_dirpath+"/labelprint_main_dialog.db");
    if(db.open()){
...
void labelprint_main_dialog::tosave()//保存自定义的内容
{
    ...
    QSettings conf(CommonHeader::com_ini_filepath);

    conf.setValue("labelprint_goodinfo_dialog/printmodel_selfh_conttent_List", gooddialog->printmodel_selfh_conttent_List);
    conf.setValue("labelprint_goodinfo_dialog/printmodel_selfh_List", gooddialog->printmodel_selfh_List);
}
...
//    }
    ...
    float _gap = conf.value(QString("label_print/gap"), "0").toFloat();
    ...
    QString path = conf.value(QString("label_print/printpath"), "/dev/usb/lp0").toString();
    QString baudrate = conf.value(QString("label_print/baudrate"),"9600").toString();

    //!==================21
    if(model->rowCount() <= 0){
        if(errstring) * errstring = tr("没有需要打印的数据");
        //        if(pop)uinormal_message::warning(parent, tr("标价打印机:"), tr("标价打印机错误:")+(istest?tr("测试至少有一条数据"):tr("没有需要打印的数据")));
        return false;
    }
    ...
    if(printer==0){
        ...
        //        uinormal_message::warning(parent, tr("标价打印机:"), tr("标价打印机错误:")+"no printer is selected");
        return false;
    }
    if(!printer->tryOpen(path)){
        if(errstring) * errstring = tr("端口打开失败");
        //        uinormal_message::warning(parent, tr("标价打印机:"), tr("标价打印机错误:")+printer->lastError());
        return false;
    }
    ...
    case 0://POSTEK Q8/200
    {
        ...
        foreach(QVector<QString> vs, vvs){
            ...
            printer->write(printer0.toPrint(rowins, print_count,height, gap));
        }
    }
        break;
    case 1://TSC TTP-244 PLUS"
    {
        ...
        foreach(QVector<QString> vs, vvs){
            ...
            printer->write(printer0.toPrint(rowins, print_count,(height), gap,(width)));
        }
    }
        break;
    case 9://佳博GP-1224T
    ...
    case 2://SC2100(商超专用)
    {
        ...
        foreach(QVector<QString> vs, vvs){
            ...
            printer->write(printer0.toPrint(rowins, print_count));
        }
    }
        break;
    case 8://GODEX G500-U
        print_darkness = conf.value("label_print/print_darkness", 8).toString();
    case 3://ARGOX CP-2140
    {
        ...
        foreach(QVector<QString> vs, vvs){
            ...
            printer->write(printer0.toPrint(rowins, print_count, print_darkness.toInt()));
        }
    }
        break;
    case 4://佳博GP-3120TN
...
{
    return getPrintMap((CommonHeader::com_root_dirpath+"/def.txt"), map, errstring);
}
...
{
    return savePrintMap(CommonHeader::com_root_dirpath+"/def.txt" , map, errstring);
}
