    ...
    ui(new Ui::labelprint_set_dialog)
{
    ...
    ui->spinBox_darkness->setValue(_Settings.value("label_print/print_darkness", 8).toInt());
...
void labelprint_set_dialog::tosave()
{
    ...
    _Settings.setValue("label_print/print_darkness", ui->spinBox_darkness->value());
}
...
void labelprint_set_dialog::test_labelprint()
{
    ...
    //!================11
    if(printer==0){
        uinormal_message::warning(this, tr("标价打印机:"), tr("标价打印机错误:")+"no printer is selected");
        return;
    }
    if(!printer->tryOpen(path)){
        uinormal_message::warning(this, tr("标价打印机:"), tr("标价打印机错误:")+printer->lastError());
        return;
    }
    ...
    case 0://! POSTEK Q8/200 测试通过 字体随意 串口成功9600
    {
        ...
        printer->write(printer0.toPrint(ins, printcount.toInt(),248,40));
    }
        break;
    case 1://! TSC TTP-244 PLUS 测试通过 字体FONT001 串口成功9600  提示： toprint的高度不能为0，会认为时无限大
    {
        ...
        printer->write(printer0.toPrint(ins, printcount.toInt(),30,0,40));
    }
        break;
    case 9://佳博GP-1224T
    {
    ...
    case 2://! SC2100(商超专用)测试通过 无串口
    {
        ...
        printer->write(printer0.toPrint(ins, printcount.toInt()));
    }
        break;
    ...
    case 8://GODEX G500-U
    {
        ...
        if(ui->spinBox_darkness->isHidden()) {
            ...
            printer->write(printer0.toPrint(ins, printcount.toInt(), ui->spinBox_darkness->value()));
        }
...
void labelprint_set_dialog::update_labelprint_path(const QString &pathtype)
{
    ...
    case 2: //网口
    {
        ...
        if(model->rowCount()>=3
            ...
            uinormal_message::warning(this, tr("标价打印机:"), tr("标价打印机错误:")+tr("不支持网口,返回串口模式"));
