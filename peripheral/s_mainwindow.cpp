      ...
      ui(new Ui_S_Mainwindow)
{
    ...
    ui->toolBar->addWidget(new QLabel(tr("x:")));
    ui->toolBar->addWidget(xline = new QLineEdit);
    ui->toolBar->addWidget(new QLabel(_danwei));

    ui->toolBar->addWidget(new QLabel(tr("y:")));
    ui->toolBar->addWidget(yline = new QLineEdit);
    ui->toolBar->addWidget(new QLabel(_danwei));

    ui->toolBar->addWidget(new QLabel(tr("宽度:")));
    ui->toolBar->addWidget(wline = new QLineEdit);
    ui->toolBar->addWidget(new QLabel(_danwei));

    ui->toolBar->addWidget(new QLabel(tr("高度:")));
    ui->toolBar->addWidget(hline = new QLineEdit);
    ui->toolBar->addWidget(new QLabel(_danwei));

    ui->toolBar->addWidget(new QLabel(tr("字体大小:")));
    ui->toolBar->addWidget(fsline = new QComboBox);

    ui->toolBar_2->addWidget(istext = new QCheckBox(tr("打印文本")));

    QLabel *strech_label;
    ui->toolBar_2->addWidget(strech_label = new QLabel);
    ui->toolBar_2->addWidget(new QLabel(tr("旋转")));
    ui->toolBar_2->addWidget(com_rotate = new QComboBox);

    fsline->setEditable(true);
    fsline->addItem(tr("标准")+"(11)", "11");
    fsline->addItem(tr("倍高")+"(12)", "12");
    fsline->addItem(tr("倍宽")+"(21)", "21");
    fsline->addItem(tr("倍高倍宽")+"(22)", "22");
...
void S_MainWindow::initDocument(const SSRPairMap &ssrmap)
{
    ...
    _yoff = conf.value("label_print/height", "320").toInt();
    _xoff = conf.value("label_print/width", "720").toInt();

    //将分辨率转为 200dpi dots 的模式
    _danwei = conf.value("label_print/danwei", "dots").toString();
    ...
    //RENEW ALL
    int w = conf.value("label_print/width", "720").toFloat();
    int h = conf.value("label_print/height", "320").toFloat();
...
void S_MainWindow::addtwobarcode()
{
    ...
    for(int c=0,count=printmodel->columnCount();c<count;c++)headerlabels.append(printmodel->horizontalHeaderItem(c)->text());
    dialog.setup(headerlabels);
    dialog.setexplaintext(tr("打印二维码"));
    if(QDialog::Accepted !=dialog.exec()) return;
    QString key = dialog.whoischecked();
    if(key.isEmpty()) return;
...
void S_MainWindow::addonebarcode()
{
    ...
    for(int c=0,count=printmodel->columnCount();c<count;c++)headerlabels.append(printmodel->horizontalHeaderItem(c)->text());
    dialog.setup(headerlabels);
    dialog.setexplaintext(tr("打印一维码"));
    if(QDialog::Accepted !=dialog.exec()) return;
    QString key = dialog.whoischecked();
    if(key.isEmpty()) return;
