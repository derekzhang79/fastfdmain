...
void lds_toolbar::setup(const QStringList &names, Qt::Alignment flag, lds_keytoolbutton::KEYTYPE type)
{
    if(this->layout()){
    ...
    layoutaddwidget(layout, names0, flag, type);
...
void lds_toolbar::layoutaddwidget(QBoxLayout *layout, const QStringList &names, Qt::Alignment flag, lds_keytoolbutton::KEYTYPE type)
{
    if(type != lds_keytoolbutton::NOKEY) {
        //unix  且 旧系统
#if defined(Q_OS_UNIX) && !defined(QT_NEW_SYSTEM)
        layout->addWidget(new lds_keytoolbutton(this, type));
#else
        /*不加*/
#endif
    }
