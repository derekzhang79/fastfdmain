...
QString n_func::f_m_deposit(QWidget *parent, QString as_memberno, double adec_deposit, double adec_realamount, double adec_amount, QString as_deposit_mode, QString as_pay_mode, QString as_memo,const QString &type)
{
    ...
    ls_operid=public_sql::gs_operid;

    if(w_m_member_list::is_yun_member){
        ch_member_belong_state = "Y";
        if(type=="C"){
            if(!w_sys_manage_cloudsync::get_yun_member_fetch(parent, as_memberno,public_sql::gs_operid,QString::number(-adec_deposit), as_pay_mode)){
                return tr("云会员取款失败");
            }
        } else if(type=="D"){
            ...
            if(!w_sys_manage_cloudsync::get_yun_member_recharge(parent, as_memberno,public_sql::gs_operid,QString::number(adec_deposit)
...
*/
            ...
            case 2://消费额服务费率
                ...
                if(ls_serviceflag == "Y") {
                    lds_query query;
                    query.execSelect(QString("select ifnull( sum( ((num_num-num_back)*num_price+num_price_add) *int_discount *0.01 ) , 0 )"
                                             //                                       INTO :ldec_cost
...
bool n_func::f_check_bus_grant(const QString &operid, const QString &opercode)
{
    ...
    lds_query query;
    query.execSelect(QString("select * from cey_sys_oper_purview where ch_purviewno = '%1' and vch_operID='%2'")
                     .arg(opercode)
                     .arg(operid)
                     );
    return query.next();
...
{
    QString is_operid = ls_operid;

    while(false == n_func::f_check_bus_grant(is_operid, opercode)){
        if(1 ==  lds_messagebox::warning(parent, MESSAGE_TITLE_VOID, tr("操作员权限不足"), tr("提升权限"), tr("取消") )) {
            return false;
        }
        lds_dialog_input inputdialog(parent);
        inputdialog.showSecondLineEdt();
        inputdialog.setWindowTitle(tr("提升权限"));
        inputdialog.ui->label->setText(tr("用户名") + ":");
        inputdialog.ui->label_2->setText(tr("密码") + ":");
        ...
        is_operid = inputdialog.ui->lineEdit->text();
        QString errstring = f_confirm_user_pwd(is_operid, inputdialog.ui->lineEdit_2->text());
...
QString n_func::READCARDINFO::desc()
{
    ...
    if(!vch_memberno.isEmpty()) {
        ...
        if(ch_favourable == "2") {
            ...
            if("1" == public_sql::discount_off_mode()){
                discount = 100 - discount;
            }
            retv += tr("固定折扣")+QString("(%1)").arg(discount);
        }
    }
    return retv;
}
...
///////////
QString n_func::f_get_sysparm_q(lds_query &query, const QString &as_parameter, const QVariant &as_default)
{
...
QString n_func::f_set_sysparm_q(lds_query &query, const QString &as_parameter, const QVariant &as_value, const QString &as_memo)
{
    qlonglong ll_ret;
