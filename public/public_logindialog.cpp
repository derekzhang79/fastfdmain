...
{
    Printer_POS_SERIAL::SERIAL_PORT_MUTEX_COM_MAP_DELETE_ALL();
    delete ui;
}
...
void public_loginDialog::init(const QString &u, const QString &p)
{
    ...
    connect(&printsizetimer,SIGNAL(timeout()),this,SLOT(toprint_activesize()));
    //
...
void public_loginDialog::goto_pre()
{
    ...
    case 0:
    {
        ...
        if(query.next()) {
            if(0 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("快餐上次反结账未完成，是否继续"), tr("继续"), tr("取消"))) {
                business_documents_query::doUnCheck(w_scr_dish_main::Q000, query.recordValue("ch_billno").toString());
            }
        }
...
{
    w_scr_dish_restaurant_div_pop::cey_bt_table_lock(ch_tableno, 0);

    w_scr_dish_main_table_800x600 dialog(ch_tableno, ch_billno, "", 0);
    dialog.move(0, 0);
    if(QDialog::Accepted ==dialog.exec()){//没有付款或付款失败/取消
        w_scr_dish_restaurant_div_pop::cey_bt_table_update(ch_tableno, "", 0);
    }
    w_scr_dish_restaurant_div_pop::cey_bt_table_unlock(ch_tableno, 0);
    return true;
}
...
void public_loginDialog::connectdb_init(lds_messagebox_loading_show *loading)
{
    ...
    if(public_sql::_d.toInt() == 0) {
        ...
        //    loading->setText(tr("正在数据库校对..."));
        public_sql::Other_sql_update();//不会影响前他升级
        if(sql_error.isEmpty()) {
        } else {
            loading->setText(tr("数据库升级:")+sql_error, 500);
        }
    }
    loading->setText(tr("初始化..."));
    //若是sd卡，备份
    ...
    if(lds::sysconf->value("w_sys_manage_sqlcopy_backup_strategy/sql_start_backup", false).toBool()){
        if(0 == lds_messagebox::question(this, MESSAGE_TITLE_VOID,  tr("开机备份") +"\n"+ tr("是否备份数据库?"), tr("确定") ,tr("取消"))){
...
void public_loginDialog::showprinterr(const QString &errstring, const QString &title, int error_id, const QString &int_print_id)
{
    ...
    backheader::static_dialogshow(*dialog, "",QSize(), Qt::AlignRight|Qt::AlignTop);
    ...
    if(sqldt.isValid()) {//数据库定时备份
        ...
        if(lds::sysconf->value("w_sys_manage_sqlcopy_backup_strategy/sql_time_backup", true).toBool()
            ...
            if((sqldt.secsTo(sqlbackupdt)/60) == 0){//相差一分钟
                ...
                if(0 == lds_messagebox::question(this, MESSAGE_TITLE_VOID,  tr("定时备份") +"\n"+ tr("是否备份数据库?"), tr("确定") ,tr("取消"), "","", true)){
                    w_sys_manage_sqlcopy::static_tobackup(this);
                }
            }
        }
        ...
        if(!lds_thread::getThreadDbAndCreat().open() ){
            lds_messagebox::information(this, MESSAGE_TITLE_VOID,  tr("数据库") +"\n"+ tr("连接失败"));
        }
    }
    /////////////////////////////////////////////////////////
}
...
{
    if(QDialog::Accepted == public_sql::MySQL_PARAME_SET(this)) {
...
void public_loginDialog::hideall_and_back_login(const QString &p, const QString &u)
{
    ...
    if(p.isEmpty() && u.isEmpty()){
        _u=public_sql::gs_operid;
        _p=public_sql::gs_password;
    }
...
{
    hideall_and_back_login(public_sql::gs_password,public_sql::gs_operid);
}
...
bool public_loginDialog::connectdb_init_check(lds_messagebox_loading_show *loading)
{
    ...
    if(!path.isEmpty()) {
        switch(lds_messagebox::critical(this, MESSAGE_TITLE_VOID,  path + tr("空间不足") +","+tr("请及时清理"), tr("打开文件管理器"), tr("继续"), tr("退出"))) {
        case 0:
    ...
    if(sql_isopen) {
        ...
        if(query.recordValue(0).toString().compare("4.0") >= 0) {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("V3软件无法连接4.0或4.0以上的数据库,请核实"));
            return false;
        }
        //~V3软件不能连接4.0以上的数据库
#endif
        ...
        if(false == query.next()) {
            ...
            int_id ++;
            if(int_id > 255) {
                lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("局域网最多支持255台机器"));
                return false;
            }
        ...
        query.execSelect(QString(" select int_id from cey_sys_lan_terminal where vch_terminal = '%1' ")
                         .arg(lds::terminalCode));
        query.next();
        *(QString *)&lds::LOCAL_TERMINAL_INDEX_FF = QString().sprintf("%02X",  query.record().value("int_id").toInt());
        //~联机多台及其根据机器号不重单付款
        //SaaS 最大连接数
        //        if(saas_pay_detail::isSaaS())
        //        {
        ...
        //                                        tr("超过最大连接数") + "\n" +
        //                                        tr("局域网机器个数")+":"+QString::number(saas_pay_detail::saas_lan_num()));
        //                return false;
        //            }
        //        }
...
QString public_loginDialog::toTryLogin()
{
    ...
    *(QString *) &public_sql::gs_operid=ls_operid;
    *(QString *) &public_sql::gs_opername=n_func::f_get_opername(ls_operid);
    *(QString *) &public_sql::gs_password=ls_password;
    ...
    if(n_func::f_get_sysparm("ftptp_use") == "1") {
        ...
        while(last_update_date < cur_date){
            ...
            if(w_scr_dish::huarun_ftp_server_success == ret
                ...
                loading->setText(tr("正在营业数据上传...")+last_update_date.toString("yyyy-MM-dd"));
                last_update_date = last_update_date.addDays(1);
                ls_err = "";
                continue;
            }
            if(w_scr_dish::huarun_ftp_file_exists == ret) {
                if(0 == lds_messagebox::warning(this, MESSAGE_TITLE_VOID,  tr("正在营业数据上传") +"\n"+  ls_err+","+tr("是否继续上传"), tr("确认"), tr("取消"))) {
        ...
        if(!ls_err.isEmpty()) {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,  tr("正在营业数据上传") + "\n" +  ls_err);
        }
        //
...
void public_loginDialog::runStart()
{
    ...
    ddata.m_hlabels = tr("名称") +","+ tr("价格") +","+ tr("数量") +","+ tr("金额");
