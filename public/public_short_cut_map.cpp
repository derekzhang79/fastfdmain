...
void public_short_cut_map::OPER_KEY_MODEL_INIT()
{
    ...
    if(public_sql::OPER_KEY_MODEL.rowCount() == 0) {
        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << new QStandardItem(tr("前台设置"))
                                             << new QStandardItem
                                             << new QStandardItem
                                             << new QStandardItem
                                             );
        public_sql::OPER_KEY_MODEL.item(public_sql::OPER_KEY_MODEL.rowCount()-1, 0)->setBackground(QColor("#ff650b"));
        public_sql::OPER_KEY_MODEL.item(public_sql::OPER_KEY_MODEL.rowCount()-1, 1)->setBackground(QColor("#ff650b"));
        public_sql::OPER_KEY_MODEL.item(public_sql::OPER_KEY_MODEL.rowCount()-1, 2)->setBackground(QColor("#ff650b"));
        public_sql::OPER_KEY_MODEL.item(public_sql::OPER_KEY_MODEL.rowCount()-1, 3)->setBackground(QColor("#ff650b"));
        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << get_item_text_usr(tr("会员"), "HY")
                                             << get_item_text_usr(tr("修改"), "XG")
                                             << get_item_text_usr(tr("删菜"), "SC")
                                             << get_item_text_usr(tr("挂单"), "GD")
                                             );

        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << get_item_text_usr(tr("切换后台"), "QHHT")
                                             << get_item_text_usr(tr("锁屏"), "SP")
                                             << get_item_text_usr(tr("结算"), "JS")
                                             << get_item_text_usr(tr("折扣"), "ZK")
                                             );

        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << get_item_text_usr(tr("数量")+"-", "SUB")
                                             << get_item_text_usr(tr("数量")+"+", "ADD")
                                             << get_item_text_usr(tr("换菜"), "HC")
                                             << get_item_text_usr(tr("取重"), "QZ")
                                             );

        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << get_item_text_usr(tr("常用菜"), "CYC")
                                             << get_item_text_usr(tr("交班"), "JB")
                                             << get_item_text_usr(tr("整单取消"), "ZDQX")
                                             << get_item_text_usr(tr("赠送"), "ZS")
                                             );

        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << get_item_text_usr(tr("开钱箱"), "KQX")
                                             << get_item_text_usr(tr("会员充值"), "HYCZ")
                                             << get_item_text_usr(tr("会员取款"), "HYQK")
                                             << get_item_text_usr(tr("退菜退款"), "TCTK")
                                             );
        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << get_item_text_usr(tr("修改密码"), "XGMM")
                                             );

        //

        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << new QStandardItem(tr("付款设置"))
                                             << new QStandardItem
                                             << new QStandardItem
                                             << new QStandardItem);
        public_sql::OPER_KEY_MODEL.item(public_sql::OPER_KEY_MODEL.rowCount()-1, 0)->setBackground(QColor("#ff650b"));
        ...
        public_sql::OPER_KEY_MODEL.item(public_sql::OPER_KEY_MODEL.rowCount()-1, 3)->setBackground(QColor("#ff650b"));
        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << get_item_text_usr(tr("微信"), "WX")
                                             << get_item_text_usr(tr("支付宝"), "ZFB")
                                             << get_item_text_usr(tr("银行卡"), "YHK")
                                             << get_item_text_usr(tr("会员卡"), "HYK")
                                             );
        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << get_item_text_usr(tr("充值"), "CZ")
                                             << get_item_text_usr(tr("代金券"), "DJQ")
                                             << get_item_text_usr(tr("美团"), "MT")
                                             << get_item_text_usr(tr("大众点评"), "DZDP")
                                             );
        public_sql::OPER_KEY_MODEL.appendRow(QList<QStandardItem*>()
                                             << get_item_text_usr(tr("全场折扣"), "QCZK")
                                             << get_item_text_usr(tr("优惠"), "YH")
                                             << get_item_text_usr(tr("会员挂账"), "HYGZ")
                                             );
#ifdef QT_DEBUG
...
void public_short_cut_map::took()
{
    ...
    foreach(const QModelIndex &index, indexs) {
        ...
        foreach(const QModelIndex &index0, indexs) {
            ...
            if(keytext == keytext0) {
                lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("按键冲突")+":" +m->data(index, Qt::DisplayRole).toString() +"," + m->data(index0, Qt::DisplayRole).toString());
                return;
            }
        }
    }
