...
//2
QList<tableSTATE> restraurantDelegate::tablestatelist;
static tableSTATE static_gettableState(tableState state){
    tableSTATE tablestate;
    foreach(const tableSTATE &t, restraurantDelegate::getTablestatelist()){
        if(t.state == state){
            tablestate = t;
            break;
        }
...
static QPainterPath getRoundRectPath(const QRectF &rect, qreal leftTopRadius, qreal leftBottomRadius, qreal rightBottomRadius, qreal rightTopRadius){
    ...
    path.arcTo(QRectF(rect.left(), rect.top() , 2 * leftTopRadius, 2 * leftTopRadius), 90, 90);

    path.lineTo(rect.bottomLeft() - QPoint(0, leftBottomRadius));
    path.arcTo(QRectF(rect.left(), rect.bottom()  - 2 * leftBottomRadius , 2 * leftBottomRadius, 2 * leftBottomRadius), 180, 90);

    path.lineTo(rect.bottomRight() - QPoint(rightBottomRadius, 0));
    path.arcTo(QRectF(rect.right() - 2 * rightBottomRadius, rect.bottom()  - 2 * rightBottomRadius, 2 * rightBottomRadius, 2 * rightBottomRadius), 270, 90);

    path.lineTo(rect.topRight() + QPoint(0, rightTopRadius));
    path.arcTo(QRectF(rect.right() - 2 * rightTopRadius, rect.top() , 2 * rightTopRadius, 2 * rightTopRadius), 0, 90);
...
void restraurantDelegate::painterCell(QPainter *painter, cell_content cell[][3], int x, int y, const QRect &rect, bool  painter_linear, bool gray_mask, qreal xyRadius, qreal  pattern_scale) {
    ...
    QRectF rect1 = QRectF(rect.x() * 1.0 + rect.width() * x / 3.0, rect.y() * 1.0 + rect.height()*y/3.0, rect.width() * 1.0 / 3, rect.height()*1.0 / 3);
    ...
    if(celldata.isdisabled){//置灰
        pg = QPixmap(":/image/widgets_pic/disable_radio_on.png");
        ...
        pg = QPixmap(":/image/widgets_pic/radio_on.png");
    ...
    if(celldata.ischecked) {
        pg = pg.scaledToWidth(20, Qt::SmoothTransformation);
        painter->drawPixmap(QStyle::alignedRect(Qt::LeftToRight, Qt::AlignCenter, pg.size(), rect1.toRect()), pg);
    }
}
...
void restaurantTableView::setTextColor(const QString &c)
{
    textColor = c;
}

QString restaurantTableView::getTextColor()
{
    return textColor;
}
...
{
    QString ch_billnos = m->data(index, Qt::UserRole+2).toString();
...
void restaurantTableView::toshowpop(const QModelIndex&index)
{
    ...
    connect(_9Box, SIGNAL(signal_div_check(QString)), this, SLOT(tocurindex_checklist(QString)));
    //同步选中
    ...
    _9Box->move(lds::getPopPoint(pos_bottom, pos_top, size, Qt::LeftToRight));
    _9Box->raise();
    _9Box->show();
}
...
{
    return static_gettableState(state);
}
...
QList<tableSTATE> &restraurantDelegate::getTablestatelist()
{
    if(tablestatelist.isEmpty()){
        tablestatelist.append(tableSTATE(tr("正使用"), tableState_KAITAI, "#c94b01"));
        tablestatelist.append(tableSTATE(tr("已预定"), tableState_YUDING , "mediumpurple"));
        tablestatelist.append(tableSTATE(tr("未使用"),  tableState_KONGXIAN , "#50c600"));
        tablestatelist.append(tableSTATE(tr("已预打"),  tableState_YUDA , "#0070a1"));
        tablestatelist.append(tableSTATE(tr("未清台"),  tableState_BAR , "white"));
        tablestatelist.append(tableSTATE(tr("二维码"),  tableState_QR_CODE , "#ff00ff"));
        //        tablestatelist.append(tableSTATE(tr("不可选"), tableState_DISABLE , "#686868"));
    }
...
void restraurantDelegate::paintDensePattern(QPainter *painter, const QPainterPath &path, const QColor &color, Qt::BrushStyle pattern, qreal pattern_scale, bool drawPen)
{
    ...
    QPen p = painter->pen();
    //scale
    QBrush pattern_brush(color, pattern);
...
restraurantDelegate::restraurantDelegate(QObject *parent):QItemDelegate(parent)
{
//    effect_type = effect_border;
    preprinttablemap = 0;
    model_cache = new fexpandmain_model_sqltablemodel;
}
...
void restraurantDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    QRect parent_rect=rect.adjusted(table_border, table_border, -table_border, -table_border);
    ...
    ///头
    //空闲
    tstate= static_gettableState( tableState_KONGXIAN);
    //预定
    if(!order_tableno.isEmpty()){
        tstate= static_gettableState(tableState_YUDING);
    }
    /////////未分单
    if(false == rect_is_div) {
        //开台
        if(ch_billno.length() > 0) {
            tstate= static_gettableState(tableState_KAITAI);
        }
        if(ch_billno.startsWith("R")) {//二维码点单
            tstate = static_gettableState(tableState_QR_CODE);
        }
        //预打单
        if(ch_billno.length() > 0) {
            if(preprinttablemap->value(ch_billno, false)) {
                tstate= static_gettableState( tableState_YUDA);
            }
        }
        //酒吧 未清台
        if(div_bar.count() > 0) {
    ...
    if(true == rect_is_div) {
        ...
        for(int k = 0; k < 9; k ++) {
            ...
            cell[sqr_2 % 3][sqr_2 / 3].showed_color = static_gettableState(tableState_KAITAI).colorname1;
            if(preprinttablemap->value(billno, false)){//预打单
                cell[sqr_2 % 3][sqr_2 / 3].showed_color = static_gettableState( tableState_YUDA).colorname1;
            }
        }
        //酒吧 未清台
        for(int k = 0; k < 9; k ++) {
    ...
    //    painter->fillPath(getRoundRectPath(parent_rect, xyRadius, xyRadius, xyRadius, xyRadius), QColor("white"));
    //pie
    bool gray_mask = false;
    if(option.state & QStyle::State_Selected) {
        //        if(effect_type == effect_border) {
        gray_mask = true;
        //        }
    }
    for(int x = 0; x < 3; x++){
...
//            //            painter->drawPath(getRoundRectPath(parent_rect.adjusted(1, 1, -1, -1), xyRadius, xyRadius, xyRadius, xyRadius));
//            break;
//        case effect_radio:
//        {
...
//void restraurantDelegate::update_select_effect(restraurantDelegate::selectEffect effect_type)
//{
...
void restraurant9BoxDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if(isdisabled) {
        tstate= static_gettableState(tableState_DISABLE);
    }
    //border
...
QString restraurantDelegateTableState::paint_transfer(const QVariant &dd) const
{
