...
void restraurantDelegate_list_table::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    if(option.state & QStyle::State_Selected) {
        painter->fillRect(option.rect, QColor("#ff650b"));
    ...
    if("-1" == key) {
        painter->drawText(option.rect, Qt::AlignVCenter | Qt::AlignHCenter , "---" + tr("上一页") + "---");
        return;
    }
    if("-2" == key) {
        painter->drawText(option.rect, Qt::AlignVCenter | Qt::AlignHCenter , "---" + tr("下一页") + "---");
        return;
    }
    if("-3" == key) {
        painter->drawText(option.rect, Qt::AlignVCenter | Qt::AlignHCenter , "---" + tr("新增") + "---");
        return;
    }
...
void restraurantDelegate_list_dish::drawText(QPainter *painter, QStyle::State state, const QRect &painter_text_rect, Qt::Alignment displayAlignment, const QString &painter_text) const
{
    if(painter_text.contains(":")) {
        ...
        QRect rect = QStyle::alignedRect(Qt::LeftToRight, displayAlignment, QSize(fm.width(painter_text), fm.height()), painter_text_rect);
