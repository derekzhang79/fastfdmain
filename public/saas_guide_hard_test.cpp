...
{
    Printer_POS *printer = Printer_POS::get_pos_printer_by_path(ui->comboBox_printer->currentText());
    Printer_POS_auto_destroy d(printer);

    if(0 == printer) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("打印机不存在"));
        return;
    }
...
void saas_guide_hard_test::todisplay_test()
{
    ...
    if(sqr%5==0){
        ...
        if(!public_sql::display_led.writeCom(sqr%5, "88888888", path)){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,public_sql::display_led.lastError());
        }
    }
    sqr ++;
...
void saas_guide_hard_test::toprinter_manage()
{
    ...
    dialog.initData("01", "P01", 0, 0, ui->comboBox_printer->currentText());
...
{
    backheader::syn_dataprin_test_byresolveself(ui->comboBox_printer->currentText(), 0, "", 0, 4);
}
