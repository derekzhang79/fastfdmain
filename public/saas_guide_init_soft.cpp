...
void saas_guide_init_soft::on_pushButton_connect_MySQL_clicked()
{
    ...
    while(false == (sql_is_connected = public_sql::CONNECT_MySQL(&loading)) ) {
        if(QDialog::Rejected == public_sql::MySQL_PARAME_SET(this)) {
            break;
        }
    }
...
void saas_guide_init_soft::on_pushButton_demo_clicked()
{
    ...
    if(1 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, ui->pushButton_demo->text(), tr("确认"), tr("取消"))) {
        return;
    }
    lds_messagebox_loading_show loading(this, "");
    loading.show_delay();
    if(!public_sql::MySQL_restore(public_sql::show_chinese_sql_file_dir, public_sql::show_chinese_sql_file,  &loading, ui->pushButton_demo->text())) {
...
bool saas_guide_init_soft::MySQL_isConnected()
{
    ...
    lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("请先连接数据库"));
    return false;
}
