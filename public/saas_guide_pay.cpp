...
{
    ui->setupUi(this);
    ui->comboBox->addItem(tr("1年"), 12);
    ui->comboBox->addItem(tr("2年"), 24);
    ui->comboBox->addItem(tr("3年"), 36);

    ui->comboBox->addItem(tr("1个月"), 1);
    ui->comboBox->addItem(tr("2个月"), 2);
    ui->comboBox->addItem(tr("3个月"), 3);
    ui->comboBox->addItem(tr("4个月"), 4);
    ui->comboBox->addItem(tr("5个月"), 5);

    ui->comboBox->addItem(tr("6个月"), 6);
    ui->comboBox->addItem(tr("7个月"), 7);
    ui->comboBox->addItem(tr("8个月"), 8);
    ui->comboBox->addItem(tr("9个月"), 9);
...
#endif
        //terminalReturn
        cJSONHttpData httpd = cJSON_CreateObject();
        cJSON_AddItemToObject(httpd.json ,"trade_no", cJSON_CreateString(dialog.out_trade_no.toLocal8Bit()));
        cJSON_AddItemToObject(httpd.json ,"extra_common_param", cJSON_CreateString(payInfo.extra_common_param.toLocal8Bit()));
        QString param = httpd.jsonToString();
        QMap<QString, QString> headinfo;
        httpd.initRef() =  w_sys_manage_cloudsync::get_yun_httppos(this,
                                                                   "http://"+saas_pay_detail::saas_Host()+"/cloudTerminal/terminalReturn.terminal",
                                                                   param,
                                                                   headinfo,
                                                                   tr("提示"),
                                                                   true,
...
void saas_guide_pay::getUpgradeService()
{
    ...
    cJSONHttpData httpd = cJSON_CreateObject();
    cJSON_AddItemToObject(httpd.json ,"terminalCode", cJSON_CreateString(saas_pay_detail::terminalCodeGet().toLocal8Bit()));
    QString param = httpd.jsonToString();
    QMap<QString, QString> headinfo;
    httpd.initRef() = w_sys_manage_cloudsync::get_yun_httppos(this,
                                                              "http://"+saas_pay_detail::saas_Host()+"/cloudTerminal/getUpgradeService.terminal",
                                                              param,
                                                              headinfo,
                                                              tr("提示"),
                                                              true,
    ...
    if(200 == httpd.httpStatus) {
        ...
        ui->lineEdit_terminal_num->setText("1");
        ui->label_7->setText(tr("服务到期") + ":" + cJSON_help::getcJSONvalue(httpd.json, "dateEnd").toString() + "," +
                             tr("服务剩余")+":" + cJSON_help::getcJSONvalue(httpd.json, "month").toString() + tr("月"));
    }
}
...
void saas_guide_pay::updatePayNum()
{
    ...
    cJSONHttpData httpd = cJSON_CreateObject();
    cJSON_AddItemToObject(httpd.json ,"terminalCode", cJSON_CreateString(saas_pay_detail::terminalCodeGet().toLocal8Bit()));
    cJSON_AddItemToObject(httpd.json ,"serviceSn", cJSON_CreateNumber(serviceSn));
    cJSON_AddItemToObject(httpd.json ,"nums", cJSON_CreateNumber(nums));
    cJSON_AddItemToObject(httpd.json ,"months", cJSON_CreateNumber(ui->comboBox->curusrdata().toInt()));
    QString param = httpd.jsonToString();

    QMap<QString, QString> headinfo;
    httpd.initRef() =  w_sys_manage_cloudsync::get_yun_httppos(this,
                                                               "http://"+saas_pay_detail::saas_Host()+"/cloudTerminal/getSelectServiceSubtotal.terminal",
                                                               param,
                                                               headinfo,
                                                               tr("提示"),
                                                               true,
...
{
    ui->toolButton_2->setText(tr("升级专业版"));
...
void saas_guide_pay::on_toolButton_num_key_clicked()
{
    ...
    if(QDialog::Accepted == d.exec()) {
        if(d.retNum().toInt() >= 1
            ...
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("终端数必须在1到999之间,包含1和999"));
        }
    }
}
