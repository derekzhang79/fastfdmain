    ...
    ui(new Ui::back_anti_checkout)
{
    ...
    model_top->setHorizontalHeaderLabels(QStringList() <<tr("序号")<<tr("单号")<<tr("结帐号")<<tr("消费总额")<<tr("操作员")<<tr("时间")<<tr("新结帐号")<<tr("桌号")<<tr("点菜人"));
    ...
    model_left->setHorizontalHeaderLabels(QStringList() <<tr("行号")<<tr("品号")<<tr("品名")<<tr("单位")<<tr("价格")<<tr("加价")<<tr("数量")<<tr("折扣")<<tr("税率")<< tr("税率") << tr("金额") << tr("赠送")<<tr("特价"));
    ...
    model_right->setHorizontalHeaderLabels(QStringList() <<tr("付款方式")<<tr("付款金额")<<tr("找零")<<tr("实收")<<tr("面额")<<tr("凭证"));
...
void back_anti_checkout::on_tableView_clicked(const QModelIndex &index)
{
    ...
    if(model_top->rowCount() > 0)
    {
        ...
        for(int i = 0; i < model.rowCount(); i++)
        {
            ...
            QString zhe = tr("折扣") +":" +model.record(i).value(6).toString() + "%";
