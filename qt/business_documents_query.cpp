...
void business_documents_query::show_login_data_by_sql()
{
    ...
    lds_query query;
    query.execSelect(QString("select * from cey_bt_dish_type_oper where vch_operid = '%1' ;").arg(public_sql::gs_operid));
    is_show_login_data = query.next();
}

void business_documents_query::right_check_and_init()
{
    if(false == backheader::operid_right_code(this, "0201")) {
        this->reject();
        return;
    }
    torefresh();
...
void business_documents_query::torefresh()//检索
{
    ...
    model_top->setQuery(
                (
                    ...
                    "(Select sum(int_person) from cey_u_table where ch_billno = cey_u_checkout_master.ch_billno) as int_person,"
                    "num_cost,"
                    "num_present,"
                    "num_discount,"
                    "num_rate,"
                    ...
                    +(is_show_login_data ? QString(" and vch_operid = '%1' ").arg(public_sql::gs_operid) : QString(""))
    ...
    ui->tableView->tableView->restore(0);
    ui->tableView->refreshSizeForContent();
    ...
    //
    //    totableview(ui->tableView->tableView->currentIndex());
}
...
void business_documents_query::totableview(const QModelIndex &index)
{
    ...
    if(ui->tableView_2->itemDelegateForColumn(model_left->fieldIndex("vch_print_memo")) == 0) {
        ui->tableView_2->setItemDelegateForColumn(model_left->fieldIndex("vch_print_memo"), new lds_model_sqltablemodel_delegate_com(this, "vch_print_memo"));
    }
    if(ui->tableView_2->itemDelegateForColumn(model_left->fieldIndex("ch_suitflag")) == 0) {
        ui->tableView_2->setItemDelegateForColumn(model_left->fieldIndex("ch_suitflag"), new lds_model_sqltablemodel_delegate_com(
                                                      this,
                                                      qrtVariantPairList()
                                                      << qrtVariantPair("P", tr("主套菜"))
                                                      << qrtVariantPair("Y", tr("子套菜"))
                                                      ));
    }
...
void business_documents_query::toprint2()//打印
{
    ...
    if(row>=0){
        ...
        print_ch_billno.toprint(lds::sysconf->value("system_setting/printerpath").toString(), 1);

        //showInsTableview(print_ch_billno.getPintIns());
    }
}
        ...
        if("01" == KenShuChooseCase_1){
            ...
            if(ldec_point != 0){
                if(!query.execInsert("t_m_point",qrtVariantPairList()
                                     ...
                                     << qrtVariantPair("vch_memo", tr("反结帐"))
            ...
            if(ldec_backcash != 0){
                if(!query.execInsert("t_m_deposit",qrtVariantPairList()
                                     ...
                                     << qrtVariantPair("vch_operID", as_operid)
                                     << qrtVariantPair("vch_memo", tr("反结帐充值付款返现"))
                                     << qrtVariantPair("dt_operdate", qrtFunc("curdate()")))) {
                    return -1;
                }
            }
...
bool business_documents_query::prepareUnCheck(const QString &ch_payno, const QString &ch_billno)
{
    ...
    if(!query.execUpdate("cey_u_checkout_master",
                         ...
                         << qrtVariantPair("vch_operID2", public_sql::gs_operid)
                         << qrtVariantPair("dt_operdate2", cur_dt_str),
                         qrtEqual("ch_payno", ch_payno))) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("反结账失败")+QString::number(__LINE__));
        return false;
    }
    ...
    if((ll_exist + ll_exist1) > 0){
        if(ll_exist > 0) {
            if(f_member_delete(ch_payno,public_sql::gs_operid) == -1){
                lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("反结账失败")+QString::number(__LINE__));
                return false;
            }
        }
...
bool business_documents_query::doWarin()
{
    ...
    query.execSelect(QString(" select ch_billno from cey_bt_table where ifnull(ch_billno, '') <> '' and ch_tableno = '%1' ")
                     .arg(ch_tableno));
    if(query.next()) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("指定结算单中的桌位正在使用"));
        return false;
    }
    ...
    query.execSelect(QString(" select 1 from cey_u_orderdish where (num_num - num_back) < 0 and ch_billno = '%1' limit 1 ")
                     .arg(ch_billno));
    if(query.next()) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("本单已做过退单处理"));
        return false;
    }

    if(model_left->rowCount() == 0) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("没有选中菜品"));
        return false;
    }
...
bool business_documents_query::insert_cey_u_table(const QString &ch_billno_old, const QString &ch_billno_new, const QString &cur_dt_str, const QString &vch_memo)
{
    ...
    tablemodel.model_data_set(0, "vch_operid", public_sql::gs_operid);
...
int business_documents_query::getDishSelected(QList<qlonglong> &ret_int_flowID_list)
{
    ...
    ui.widget->setup(QStringList() << tr("全选") << tr("全不选") << tr("上一页") << tr("下一页")<< tr("确定") << tr("取消"), Qt::AlignRight);
    ui.tableView->setTransferHheader();
    ...
    connect(ui.widget->index_widget(tr("确定")), SIGNAL(clicked()),&dialog,SLOT(accept()));
    connect(ui.widget->index_widget(tr("取消")), SIGNAL(clicked()),&dialog,SLOT(reject()));
    connect(ui.widget->index_widget(tr("上一页")), SIGNAL(clicked()),ui.tableView,SLOT(toPageUp()));
    connect(ui.widget->index_widget(tr("下一页")), SIGNAL(clicked()),ui.tableView,SLOT(toPageDown()));
...
bool business_documents_query::doUnCheck(const QString &ch_tableno, const QString &ch_billno)
{
    ...
    //lock table
    qDebug() << __LINE__ ;
    w_scr_dish_restaurant_div_pop::cey_bt_table_lock(ch_tableno, 0);//快餐会失败
    w_scr_dish_main_table_800x600 dialog(ch_tableno, ch_billno, "", 0);
    dialog.move(0, 0);
    qDebug() << __LINE__ ;
    if(QDialog::Accepted ==dialog.exec()){//没有付款或付款失败/取消
        //cancel table
        w_scr_dish_restaurant_div_pop::cey_bt_table_update(ch_tableno, "", 0);//快餐支持
        f = true;
    }
    qDebug() << __LINE__ ;
    //unlock
    w_scr_dish_restaurant_div_pop::cey_bt_table_unlock(ch_tableno, 0);//快餐会失败
    qDebug() << __LINE__ ;
    return f;
}
...
void business_documents_query::touncheck()
{
    ...
    /** -----------------------------权限-----------------------------begin*/
    if(!n_func::f_check_bus_grant_else_authority(public_sql::gs_operid, "0227", this, tr("提示"))){
        return;
    }
    /** -----------------------------权限-----------------------------end*/
    //快餐
    query.execSelect(QString("select ch_billno from cey_sys_lan_terminal where ch_billno <>'' and vch_terminal = '%1';")
                     .arg(lds::terminalCode));
    if(query.next()) {
        switch(lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("快餐上次反结账未完成"), tr("继续"), tr("跳过"), tr("取消"))) {
        case 0://继续
            if(false == doUnCheck(w_scr_dish_main::Q000, query.recordValue("ch_billno").toString())) {
                return;
            }
            if(1 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("继续反结账"), tr("继续"), tr("取消"))) {
                return;
            }
            break;
        case 1://跳过
    ...
    if(ch_billno_old.startsWith("Q")) {
        query.execSelect(QString("select ch_billno from cey_sys_lan_terminal where ifnull(ch_billno, '') <>'' and vch_terminal = '%1';")
                         .arg(lds::terminalCode));
        if(query.next()) {
            lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("快餐上次反结账未完成"));
            return;
        }
    }
    //~快餐
    if(1 == lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("反结账将会清除以前结算记录"), tr("继续"), tr("取消"))){
        return;
    }
    ...
    QString ch_tableno = model_top->model_data(row, "ch_tableno").toString();
    w_scr_dish_restaurant_div_pop::cey_bt_table_update(ch_tableno, ch_billno_old, 0);//快餐支持
...
void business_documents_query::torefund()
{
    ...
    if(QDialog::Accepted == getDishSelected(int_flowID_list)) {
        ...
        if(false == w_scr_dish_restaurant_div_pop::getMaxChbillnoAdd1_insert_cey_u_master(ch_billno_old.left(1), ch_billno_new)) {
            lds_query::roll_back();
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("反结账失败")+QString::number(__LINE__));
            return;
        }
        ...
        QString ch_payno_old = model_top->model_data(row, "ch_payno").toString();
        QString cur_dt_str = n_func::f_get_sysdatetime_str();
        if(false == insert_cey_u_table(ch_billno_old, ch_billno_new, cur_dt_str, ch_payno_old+tr("提示"))) {
            lds_query::roll_back();
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("反结账失败")+QString::number(__LINE__));
            return;
        }
        ...
        tablemodel.setEditStrategy(lds_model_sqltablemodel::OnManualSubmit);
        tablemodel.select(ch_billno_new, this, true);
        foreach(int int_flowid, int_flowID_list) {
            ...
            int qcount = query.recordCount();
            for(int q = 0; q < qcount; q++) {
                if(tablemodel.fieldIndex("int_flowid") == q) continue;
            ...
            tablemodel.model_data_set(sqr, "vch_operid", public_sql::gs_operid);
