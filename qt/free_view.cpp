    ...
    ui(new Ui::free_view)
{
    ...
    ui->lineEdit_3->setText(public_sql::gs_operid);
    ui->lineEdit_4->setText(public_sql::gs_password);

    if(ch_paymodeno != "ZZ") {
        ui->label_discount->hide();
        ui->spinBox_discount->hide();
    } else {
        ui->label_2->setText(tr("优惠"));
    }
...
void free_view::on_pushButton_ok_clicked()
{
    ...
    {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("操作员代码不能为空"));
        return;
    }
    ...
    {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("操作金额不能大于结算金额"));
        return;
    }
    ...
    if(!m.isEmpty()){
        ...
        if(1 == lds_messagebox::warning(this, MESSAGE_TITLE_VOID
                                        ,tr("会员余额")+":"+lds::septNumber(num_amount) + "\n" + tr("挂账金额")+":"+lds::septNumber(ui->lineEdit_amount->value())
                                        ,tr("确定"),tr("取消"))){
            return;
        }
    }
