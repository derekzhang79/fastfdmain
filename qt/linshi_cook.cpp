    ...
    ui(new Ui::linshi_cook)
{
    ...
    model_dish = new ftableview_standmodel_sql(ui->tableView);
    model_dish->sql = "select CONCAT(vch_dishname,'\n',num_price), ch_dishno as dishno1, ch_dishno as dishno2   from cey_bt_dish where ch_stopflag <> 'Y' and ch_tempflag = 'Y'";
    ui->tableView->setModel(model_dish);
    model_dish->selectionmodel = ui->tableView->selectionModel();
    /// ---------------delegate -------------------begin
    //修改delegate的某些样式单独修改
    ftableview_delegate *d = new ftableview_delegate;
    ui->tableView->setProperty("outer_stylesheet", "tableView_dish");
    d->setColorLight(3);
    d->keepConnect(ui->tableView);
    ui->tableView->setItemDelegate(d);
    /// ---------------delegate -------------------end
    ui->tableView->updateFixed_fillData_addAnimation(4, 6, 100, 100);
    ...
    QTimer::singleShot(0, model_dish, SLOT(toinit()));
}
...
void linshi_cook::took()
{
    if(false == ui->tableView->currentIndex().isValid()){
        lds_messagebox::warning(this ,_TEXT_SLOT(this), tr("没有菜品被选中"));
        return;
    }
...
bool linshi_cook::dataChecked()
{
    ...
    if(0 < query.record().value(0).toInt()) {
        if(1 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("有临时菜未添加厨打方案"), tr("继续退出"), tr("取消"))) {
            return false;
        }
    }
    return true;
