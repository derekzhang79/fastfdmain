...
void lock_screen::on_pushButton_ok_clicked()
{
    ...
    {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("请输入完整的用户名和密码"));
    ...
    if(str == "1" && type == "1")
    {
        if(ui->lineEdit->text().trimmed() != public_sql::gs_operid)
        {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("锁屏解锁必须是系统登录员工,请核实"));
    ...
    if(ui->lineEdit->text().trimmed() == "0000")
    {
        ...
        {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("密码输入错误,请重新输入"));
    ...
    else
    {
        ...
        {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("用户名代码不存在，请核实"));
