    ...
    ui(new Ui::table_lock)
{
    ...
    for(int k = 0; k < 9; k++ ) {
        if(k == 0) {
            tablemodel->save_set_header("ch_billno", tr("当前单号"));
            tablemodel->save_set_header("ch_lockflag", tr("是否锁台"), true);
        } else {
            //            tablemodel->save_set_header("ch_billno_"+QString::number(k), tr("当前单号")+QString::number(k));
            tablemodel->save_set_header("ch_lockflag_"+QString::number(k), tr("是否锁台")+QString::number(k), true);
        }
    }
