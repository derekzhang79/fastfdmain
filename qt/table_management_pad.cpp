...
{
    QGraphicsItem::focusInEvent(event);
    app_item->updaste_textcolor(QColor("#ff650b"));
    this->show();
}
...
{
    QGraphicsItem::focusOutEvent(event);
    app_item->updaste_textcolor(QColor("black"));
    this->hide();
...
void table_management_pad_control::transform_app_item(const QString &text)
{
    ...
    if(text.contains("DEL")) {
        this->clearFocus();
        static_cast<table_management_graphicsview*>(app_item->scene()->views()[0])->delete_item(app_item->get_text());
        app_item = 0;
    }
}
