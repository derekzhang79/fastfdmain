...
{
    if(ch_suitflag == "Y"){
        lds_messagebox::warning(parent, MESSAGE_TITLE_VOID,tr("套菜明细不能单独修改数量"));
        return false;
    }
    if(ch_presentflag == "Y")
    {
        lds_messagebox::warning(parent, MESSAGE_TITLE_VOID,tr("已赠送的点菜记录,数量不可改"));
        return false;
    }
    return true;
}
...
void take_weight::updategetweight(const QString &value)
{
    ...
    if(ch_count == "Y") {
        if(num_count_weight != 0) {
            qreal v = value.toDouble() * 1000.0 / num_count_weight;
            ui->lineEdit_getweight->setText(QString::number(qRound(v)));//
            little_text = QString::number(v, 'f', 3) + " = " + value + "KG * 1000 /" + QString::number(num_count_weight, 'f', 3) + "KG" ;
        }
    } else {
...
void take_weight::toAutoOk(int step)
{
    ...
    ui->pushButton_ok->setText(QString("%1(%2)").arg(tr("确认")).arg(step/2));
    if(step == 0){
...
void take_weight::initData()
{
    ...
    kg2otherrate = 1;
    ui->pushButton_ok->setText(tr("确认"));
    ...
    countprice_scale->setType(lds::sysconf->value("system_setting/dianzichengxinghao").toString());
    countprice_scale->setBaudrate(lds::sysconf->value("system_setting/dianzicheng_baudrate").toInt());
    connect(countprice_scale,SIGNAL(weighComming(QString)),this,SLOT(updategetweight(QString)));
    connect(countprice_scale,SIGNAL(qupiComming(bool)),this,SLOT(quipistate(bool)));
    connect(countprice_scale,SIGNAL(lingweiComming(bool)),this,SLOT(lingweistate(bool)));
    connect(ui->lineEdit_getweight,SIGNAL(textChanged(QString)),this,SLOT(updatetotal(QString)));
    countprice_scale->tryStart_qs(lds::sysconf->value("system_setting/dianzichengduankou").toString());
