...
QString w_scr_dish::huarunInfo_error(int info)
{
    ...
    case w_scr_dish::huarun_ftp_file_exists:
        errstring = tr("服务器文件已存在");
        break;
    case w_scr_dish::huarun_ftp_server_error:
        errstring = tr("上传失败，请检查参数是否正确");
        break;
...
int w_scr_dish::XIEXIN_BUSINESS_DATA_UPLOAD(const QDate &date, QString *errstring)
{
    ...
    cJSON_help::_cJSON_AddStringToObject( root, "CUBE_ID", CUBE_ID);//收款机号
    cJSON_help::_cJSON_AddStringToObject( root, "SALES_DATE", SALES_DATE);//交易日期
    cJSON_help::_cJSON_AddStringToObject( root, "TRANS_NO", TRANS_NO_max_999999);//交易序号
    cJSON_help::_cJSON_AddStringToObject( root, "RECORD_BEGIN", RECORD_BEGIN);//交易开始时间
    cJSON_help::_cJSON_AddStringToObject( root, "RECORD_END", RECORD_END);//交易结束时间
    cJSON_help::_cJSON_AddStringToObject( root, "RECEIVER_ID", RECEIVER_ID);//收银员编号
    cJSON_help::_cJSON_AddStringToObject( root, "TRANS_TYPE", TRANS_TYPE);//交易类别码
    cJSON_help::_cJSON_AddStringToObject( root, "GROSSPLUS", GROSSPLUS);//交易金额
    cJSON_help::_cJSON_AddStringToObject( root, "DISC_AMT", DISC_AMT);//折扣金额
    cJSON_help::_cJSON_AddStringToObject( root, "NET", NET);//交易净额
    cJSON_help::_cJSON_AddStringToObject( root, "SALE_COUNT", SALE_COUNT);//交易笔数
    cJSON_help::_cJSON_AddStringToObject( root, "TOT_QTY", TOT_QTY);//交易商品件数
    cJSON_help::_cJSON_AddStringToObject( root, "VIP_NO", "");//会员编号
    cJSON_help::_cJSON_AddStringToObject( root, "ORG_GUI_DATE", "");//原交易日期
    cJSON_help::_cJSON_AddStringToObject( root, "ORG_POS_ID", "");//原交易机号
    cJSON_help::_cJSON_AddStringToObject( root, "ORG_TRANS_NO", "");//原交易序号
    ...
    while(query.next()) {
        ...
        cJSON_help::_cJSON_AddStringToObject( item, "CUBE_ID", CUBE_ID);//  收款机号
        cJSON_help::_cJSON_AddStringToObject( item, "SALES_DATE", SALES_DATE);
        cJSON_help::_cJSON_AddStringToObject( item, "TRANS_NO", TRANS_NO_max_999999);
        cJSON_help::_cJSON_AddStringToObject( item, "SELL_PRICE", SELL_PRICE);
        cJSON_help::_cJSON_AddStringToObject( item, "QTY", QTY);
        cJSON_help::_cJSON_AddStringToObject( item, "SELL_AMT", SELL_AMT);
        cJSON_help::_cJSON_AddStringToObject( item, "DISC_AMT", DISC_AMT);
        //        cJSON_help::_cJSON_AddStringToObject( item, "DISC_RATE", "0");
        cJSON_help::_cJSON_AddStringToObject( item, "SALES_AMT", SALES_AMT);
        cJSON_help::_cJSON_AddStringToObject( item, "GROUP_ID", "");
        cJSON_help::_cJSON_AddStringToObject( item, "GOODS_ID", "");
        cJSON_help::_cJSON_AddStringToObject( item, "ND", "000");//默认
        cJSON_help::_cJSON_AddStringToObject( item, "ND_TYPE", "0");//默认
    ...
    while(query.next()) {
        ...
        cJSON_help::_cJSON_AddStringToObject( item, "SALES_DATE", SALES_DATE);
        cJSON_help::_cJSON_AddStringToObject( item, "TRANS_NO", TRANS_NO_max_999999);
        cJSON_help::_cJSON_AddStringToObject( item, "TENDER",  "10");//现金支付方式
        cJSON_help::_cJSON_AddStringToObject( item, "AMOUNT", (num_cost - num_discount - num_present));
    }
