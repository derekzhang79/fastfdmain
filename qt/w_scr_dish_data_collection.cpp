...
void w_scr_dish_data_collection::torepair()
{
    ...
    if(query.recordValue(0).toLongLong() == 0){
        lds_messagebox::information(this, MESSAGE_TITLE_VOID,tr("没有需要修复的数据"));
        return;
    }
    ...
    if(sqlerr.isEmpty()){
        lds_query::com_mit();
        lds_messagebox::information(this, MESSAGE_TITLE_VOID,tr("修复成功，下次营业会依次上传"));
        return;
    }
    lds_query::roll_back();
    lds_messagebox::warning(this, MESSAGE_TITLE_VOID,tr("修复失败"));
}
