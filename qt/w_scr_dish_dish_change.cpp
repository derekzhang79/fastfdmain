    ...
    ui(new Ui::w_scr_dish_dish_change)
{
    ...
    //4
    ui->frame_2->setup(QStringList() << tr("等价商品") << tr("所有商品")<< tr("可替换菜品"), Qt::AlignRight);
    ui->label_desc->setText(tr("当前菜品(%1)将不会显示").arg(vch_dishname+"/"+_ch_dishno_cur));
    ...
    if(query.recordValue(0).toLongLong() == 0){
        ui->frame_2->index_widget(tr("可替换菜品"))->setVisible(false);
        ui->frame_2->index_widget(tr("等价商品"))->animateClick();
    } else {
        ui->frame_2->index_widget(tr("可替换菜品"))->animateClick();
        ui->label->setText(tr("可替换菜品"));
    }
    ...
    connect(ui->frame_2->index_widget(tr("等价商品")),SIGNAL(clicked()),this,SLOT(toselectequal()));
    connect(ui->frame_2->index_widget(tr("所有商品")),SIGNAL(clicked()),this,SLOT(toSelectAll()));
    connect(ui->frame_2->index_widget(tr("可替换菜品")),SIGNAL(clicked()),this,SLOT(tosuitchange()));
    ...
    ui->frame_2->index_widget(tr("等价商品"))->setShortcut(Qt::Key_E);
    ui->frame_2->index_widget(tr("等价商品"))->setShortcut(Qt::Key_A);
    ui->frame_2->index_widget(tr("等价商品"))->setShortcut(Qt::Key_R);
...
void w_scr_dish_dish_change::toselect()
{
    ...
    querymodel->setHeaderData(0, Qt::Horizontal, tr("菜品编码"));
    querymodel->setHeaderData(1, Qt::Horizontal, tr("菜品名称"));
    querymodel->setHeaderData(2, Qt::Horizontal, tr("菜品价格"));
    querymodel->setHeaderData(3, Qt::Horizontal, tr("替换比例"));
