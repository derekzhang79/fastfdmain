    ...
    ui(new Ui_w_scr_dish_group_Dialog)
{
    ...
    /// ---------------delegate -------------------end
    ui->tableView_dish->updateFixed_fillData_addAnimation(3, 6, 100, 100);
...
void w_scr_dish_group_dialog::refresh(const QString &ch_dishno)
{
    ...
    model_dish->sql = QString(
                ...
                ",a.vch_color"
                " from cey_bt_dish a "
                " where a.ch_groupno = '%1'  "
                " and a.ch_stopflag <> 'Y' and a.ch_tempflag <> 'Y' "
                )
            .arg(ch_groupno);

    this->setWindowTitle(vch_dishname + " "+ tr("组号"));
...
{
    QModelIndexList indexlist = ui->tableView_dish->selectionModel()->selectedIndexes();
    if(indexlist.isEmpty()) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("没有菜品被选中"));
        return;
    }
    ret_ch_dishno = indexlist[0].data(Qt::UserRole + 1).toString();
    this->accept();
}
