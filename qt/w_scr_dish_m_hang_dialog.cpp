...
void w_scr_dish_m_hang_Dialog::took()
{
    ...
    if(m.isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("所选会员为空"));
        return;
    }
...
void w_scr_dish_m_hang_Dialog::updatestate(const QString &vch_memberno, const QString &ch_paymodeno, const QString &ch_state, const QString &ch_cardflag, float num_amount, const QDateTime &dt_limit)
{
    ...
    if(isyun){
        if(ch_paymodeno == "02"){
            if(1 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("正在将云会员改为本地会员,是否继续"), tr("继续"), tr("取消"))){
    ...
    //1
    if(ch_state != "1"){
        ui->label_state->setText(ui->label_state->text()+"/"+tr("非正常卡"));//ch_state
    }
    if(ch_cardflag != "Y"){
        ui->label_state->setText(ui->label_state->text()+"/"+tr("未发卡"));//ch_cardflag
    }
    if(dt_limit < n_func::f_get_sysdatetime()){
        ui->label_state->setText(ui->label_state->text()+"/"+tr("超过有效期"));//ch_cardflag
    }
    if(!ui->label_state->text().isEmpty()){
        ui->label_state->setText(tr("卡状态")+":"+ui->label_state->text());
    }
    //2
    if(ui->label_state->text().isEmpty()){
