    ...
    ui(new Ui::w_scr_dish_main_bar_cancel_win)
{
    ...
    tablemodel->save_set_header("ch_dishno", tr("菜品名称"));
    tablemodel->save_set_header("num_dish", tr("数量"), 3);
    tablemodel->save_set_header("dt_enddate", tr("有效期至"), 3);
    tablemodel->save_set_header("vch_operID", tr("存酒人"));
    tablemodel->save_set_header("vch_memo", tr("备注"), true);
...
{
    QString vch_dishname = tr("菜品名称");
    QString num_dish_taken_str = tr("取出");
    QString num_dish_remain_str = tr("剩余");
    int vch_dishname_len = 17;
    int num_dish_taken_str_len = 7;
    int num_dish_remain_str_len = 7;
    int print_row = 0;
    blockIns printins;
    printins.append(backheader::printer.drawText("a00,s11", tr("取酒") +"\n","a10,s00"));
    ...
    //一键取出所选菜品
    if(row_selected.count() > 0) {
        switch(lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("一键取出所选菜品"), tr("确定"), tr("分开取"), tr("取消"))) {
        case 0://一键取出所选菜品
        {
            lds_query query;
            ...
            foreach(int row, row_selected) {
                ...
                if(false == query.execUpdate("cey_bt_table_bar_wine",
                    ...
                    lds_query::roll_back();
                    lds_messagebox::warning(this, MESSAGE_TITLE_VOID,  tr("取酒") + ":" + vch_dishname + "\n"+ query.recordError());
                    return;
                }
    ...
    foreach(int row, row_selected) {
        ...
        inputdialog.setWindowTitle(tr("取酒") + ":" + vch_dishname);
        ...
        if(QDialog::Accepted == lds_roundeddialog_rect_align(&inputdialog).exec()) {
            ...
            lds_query::com_mit();
            lds_messagebox::information(this, MESSAGE_TITLE_VOID,  tr("取酒") + ":" + vch_dishname + "\n"+ tr("操作成功"));
        }
    }
    torefresh();
...
print:
    if(print_row > 0) {
        ...
        printins.append(backheader::printer.drawText("", tr("操作时间")+":"+ n_func::f_get_sysdatetime_str() + "\n"));
...
void w_scr_dish_main_bar_cancel_win::toprint()
{
    ...
    QString vch_dishname = tr("菜品名称");
    QString num_dish = tr("数量");
    QString dt_enddate = tr("有效期至");
    int vch_dishname_len = 14;
    int num_dish_len = 7;
    int dt_enddate_len = 10;
    blockIns printins;
    printins.append(backheader::printer.drawText("a00,s11", tr("存酒") +"\n","a10,s00"));
    printins.append(backheader::printer.drawText("", ui->label_vch_guest->text()+":"+ui->lineEdit_vch_guest->text()+"\n"));
    printins.append(backheader::printer.drawText("", ui->label_vch_handle->text()+":"+ui->lineEdit_vch_handle->text()+"\n"));
    printins.append(backheader::printer.drawText("-"));
    printins.append(backheader::printer.drawText("",
    ...
    printins.append(backheader::printer.drawText("", tr("操作员")+":"+ public_sql::gs_operid+"\n"));
    printins.append(backheader::printer.drawText("", tr("操作时间")+":"+ n_func::f_get_sysdatetime_str() + "\n"));
    printins.append(backheader::printer.drawText("", tr("过期失效") + "\n"));
    printins.append(backheader::printer.drawTextEnd("1B6405 1D5601", ""));//走纸5x， 切纸
    ...
    for(int row = 0; row < tablemodel->rowCount(); row ++) {
        if(tablemodel->opera(row) == lds_model_sqltablemodel::OpInsert) {
            if(tablemodel->model_data(row, "num_dish").toDouble() <= 0) {
                lds_messagebox::warning(this, MESSAGE_TITLE_VOID,  QString::number(row+1) + tr("行") +"\n"+ tr("菜品数量不能小于等于0"));
...
w_scr_dish_main_bar_cancel_win_take::w_scr_dish_main_bar_cancel_win_take(const QString &vch_handle, QWidget *parent)
    : w_scr_dish_main_bar_cancel_win(vch_handle, parent)
{
    tablemodel->save_set_header("num_dish", tr("数量"), 0);
    tablemodel->save_set_header("vch_memo", tr("备注"), 0);
...
{
    tablename_kvmap_insert("mysql_now", "select  'cur_time', now()", false, SELECT_AUTO_REFRESH_KEY);
}
...
QVariant w_scr_dish_main_bar_cancel_win::sqlTableModel::data(const QModelIndex &idx, int role) const
{
    ...
    if(Qt::TextColorRole == role) {
        if(idx.column() == this->fieldIndex("dt_enddate")) {
            if(this->tablename_kvmap_value_direct("mysql_now", "cur_time").toDate() > data(idx).toDate()) {
                return QColor("red");
            }
            if(this->tablename_kvmap_value_direct("mysql_now", "cur_time").toDate() == data(idx).toDate()) {
