...
void w_scr_dish_restaurant_dishinfo::torefresh()
{
    ...
    for(int column = 0; column < ui->tableView->horizontalHeader()->count(); column ++ ) {
        lds_model_sqltablemodel_delegate_com *d = static_cast<lds_model_sqltablemodel_delegate_com *>(ui->tableView->itemDelegateForColumn(column));
        if(d && d->getType() >= 0) {//此处，static_cast 强转时，会变成delegate_check, 结果
    ...
    case 1:
    {
        ...
        ui->tableView->setItemDelegateForColumn(querymodel->fieldIndex("ch_state"),
                                                new lds_model_sqltablemodel_delegate_com(this, ldsVariantMap("N", tr("空闲"), "Y", tr("在用"))));
