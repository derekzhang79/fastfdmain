...
void w_scr_dish_restaurant_preorder::tochange()
{
    ...
    if(row >= 0){
        ...
        if(ch_state == "Y"){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("该餐桌已经预定,")+tr("操作取消"));
            return;
        }
        //
        w_scr_dish_restaurant_preorder_add_Dialog dialog(this);
...
void w_scr_dish_restaurant_preorder::tocancel()
{
    ...
    if(row >= 0){
        ...
        //
        if(1 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("是否取消预定"), tr("是"), tr("否"))){
            return;
        }
...
void w_scr_dish_restaurant_preorder::toprintbill()
{
    ...
    if(row >= 0){
        ...
        if(!ch_bookno.isEmpty()){
            ...
            query.next();
            blockIns printins;
            printins.append(backheader::printer.drawText("a00,s11", tr("预定详情")+"\n","a10,s00"));
            printins.append(backheader::printer.drawText("", tr("订单号:")+ch_bookno +"\n"));
            printins.append(backheader::printer.drawText("", tr("预到桌号:")+query.recordValue("vch_tables").toString()+"\n"));
            printins.append(backheader::printer.drawText("", tr("预订人:")+query.recordValue("vch_booker").toString()+"\n"));
            printins.append(backheader::printer.drawText("", tr("联系电话:")+query.recordValue("vch_tel").toString()+"\n"));
            printins.append(backheader::printer.drawText("", tr("备用电话:")+query.recordValue("vch_tel_second").toString()+"\n"));
            printins.append(backheader::printer.drawText("", tr("人数:")+query.recordValue("int_person").toString()+"\n"));
            printins.append(backheader::printer.drawText("", tr("预到时间:")+query.recordValue("dt_come").toDateTime().toString("yyyy-MM-dd hh:mm:ss")+"\n"));
            printins.append(backheader::printer.drawText("", tr("操作员:")+query.recordValue("vch_operid").toString()+"\n"));
            printins.append(backheader::printer.drawText("", tr("操作时间:")+query.recordValue("dt_operdate").toDateTime().toString("yyyy-MM-dd hh:mm:ss")+"\n"));
...
void w_scr_dish_restaurant_preorder::toorderopentable()
{
    ...
    if(row >= 0){
        ...
        if(!msg.isEmpty()){
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, msg+tr("有餐桌已经开台, 请核实"));
            return;
        }
        ...
        while(querygoing.next()){
            ...
            if(w_scr_dish_restaurant_div_pop::getMaxChbillnoAdd1_insert_cey_u_master("B", ch_billno)) {
...
void w_scr_dish_restaurant_preorder::tosearch()
{
    ...
    querymodel->setHeaderData(0, Qt::Horizontal, tr("预定人"));
    querymodel->setHeaderData(1, Qt::Horizontal, tr("餐桌名称"));
    querymodel->setHeaderData(2, Qt::Horizontal, tr("联系电话"));
    querymodel->setHeaderData(3,Qt::Horizontal, tr("人数"));
    querymodel->setHeaderData(4, Qt::Horizontal, tr("预到时间"));
    querymodel->setHeaderData(5, Qt::Horizontal, tr("预警时间"));
    querymodel->setHeaderData(6, Qt::Horizontal, tr("操作日期"));
    querymodel->setHeaderData(7, Qt::Horizontal, tr("操作员"));
    querymodel->setHeaderData(8, Qt::Horizontal, tr("备用电话"));
    querymodel->setHeaderData(9, Qt::Horizontal, tr("备注"));
    querymodel->setHeaderData(10, Qt::Horizontal, tr("是否预定"));
    querymodel->setHeaderData(11, Qt::Horizontal, tr("单号"));
}
    ...
    ui(new Ui_w_scr_dish_restaurant_preorder_add_Dialog)
{
    ...
    QDateTime dt = n_func::f_get_sysdatetime();
    //1
    ftableview_standmodel_sql *modeltype = new ftableview_standmodel_sql(ui->tableView_tabletype);
    modeltype->sql = "SELECT  concat(vch_typename,'[',ch_typeno,']') as nametype, ch_typeno  FROM cey_bt_table_type union select '"+tr("所有类型")+"' as vch_typename, '%' as ch_typeno  ORDER BY ch_typeno ASC";
    ui->tableView_tabletype->setModel(modeltype);
    modeltype->selectionmodel = ui->tableView_tabletype->selectionModel();
    ...
    ui->tableView_tabletype->updateFixed_fillData_addAnimation(5, 1, 100, 50);
    //    modeltype->toinit();
    //5
    modeltable = new ftableview_standmodel_sql_table(ui->tableView_table);
    modeltable->sql =
            ...
            restraurantDelegate::sql_ch_billnos + " as ch_billnos,"
                                                  ...
                                                  " from cey_bt_table y"
            ;
    modeltable->sqlother = modeltable->sql + " where y.ch_typeno like '%1%' " ;
    //    modeltable->sql = modeltable->sql + " group by y.ch_tableno ";
    ui->tableView_table->setModel(modeltable);
    modeltable->selectionmodel = ui->tableView_table->selectionModel();
    ...
    ui->comboBox_vch_operid->addItems(backheader::vch_operid_name);
...
void w_scr_dish_restaurant_preorder_add_Dialog::tosetovertime()
{
    ...
    dialog.setWindowTitle(_TEXT_SLOT(this));
    if(QDialog::Accepted == lds_roundeddialog_rect_align(&dialog).exec()){
        //if(0 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, tr("是否更新最大翻台时间"), tr("是"), tr("否")))
        {
            ui->spinBox_int_overtime->setValue(n_func::f_get_sysparm("table_max_over_time", "240").toInt());
        }
    }
...
{
    ui->label_tabledesc->setText(tr("当前台数")+":"+ QString::number(ui->tableView_table->get_usr_list(Qt::UserRole + 1).count()));
}
...
bool w_scr_dish_restaurant_preorder_add_Dialog::saveData()
{
    ...
    if(ui->comboBox_vch_operid->currentText().isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("预订人不能为空"));
        return false;
    }
    ...
    if(!vch_tables_failure.isEmpty()){
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("桌台%2,在最大翻台(%1分钟)范围内已经被预定\n操作失败")
    ...
    //ch_bookno 这个不管该单是否已经被预定，获取最大单号，以免重复
    if(ch_bookno.isEmpty()){
...
w_scr_dish_restaurant_preorder_set::w_scr_dish_restaurant_preorder_set(QWidget *parent):QDialog(parent), ui(new Ui_w_scr_dish_restaurant_preorder_set)
{
    ui->setupUi(this);
