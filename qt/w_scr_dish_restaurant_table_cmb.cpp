    ...
    ui(new Ui::w_scr_dish_restaurant_table_cmb)
{
    ...
    modeltable->sql = modeltable->sql.arg("");
...
void w_scr_dish_restaurant_table_cmb::took()
{
    QString errstring;
    lds_query query;
    lds_query query_going;
    lds_query query_going2;
    ...
    if(!indexlist.isEmpty()) {
        ...
        foreach(const QModelIndex &index, ui->tableView->selectionModel()->selectedIndexes()){
            QString ch_billnos = index.data(Qt::UserRole + 2/*ch_billnos*/).toString();
            QMap<int, QString> ch_billno_map = restraurantDelegate::get_ch_billno_div_map(ch_billnos);
            QStringList sqr_list = ui->tableView->getCheckList(index);

            if(restraurantDelegate::ch_billno_is_div(ch_billno_map)) {//分单
                foreach(const QString &sqr, sqr_list) {
                    cmbData cmdd;
                    cmdd.ch_billno = ch_billno_map.value(sqr.toInt());
                    cmdd.ch_tableno = index.data(Qt::UserRole + 1/*ch_tableno*/).toString();
                    cmdd.sqr = sqr.toInt();
        ...
        if(!ch_billno_from_list.isEmpty()){
            QString transferinfo =
                    ...
                    ui->pushButton_destination->text()+":\n"+ui->label_cur_table->text()        +  ( _9Box->iscombine ? "" : ( "," + tr("分单号") + indexlist[0].data(w_scr_dish_restaurant_div_pop::Model_sqr).toString() ))
                    ;
            errstring = "";
            if(0 == lds_messagebox::question(this, this->windowTitle(), transferinfo,tr("确认"), tr("取消"))){
                //state
                QString state;
                ...
                //!成功
                lds_query::com_mit();
                lds_messagebox::information(this, this->windowTitle(), tr("操作成功"));
                this->accept();
                return;
            }
        }
