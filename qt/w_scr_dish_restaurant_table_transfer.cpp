    ...
    ui(new Ui::w_scr_dish_restaurant_table_transfer)
{
    ...
    ui->label_desc->setText(tr("已开台未分单、已开台分单满单，的将不会显示"));
...
void w_scr_dish_restaurant_table_transfer::took()
{
    ...
    if(!indexlist.isEmpty()){
        ...
        if(w_scr_dish_restaurant_div_pop::cey_bt_table_has_locked(ch_tableno_from, div_sqr_from)) {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, tr("该餐桌已经被锁定,请确定"));
            return;
        }
        if(index.isValid()){
            ch_tableno_to = index.data(Qt::UserRole + 1/*ch_tableno*/).toString();
            QStringList check_to_list = modeltable->data(index, Qt::UserRole + 7/*checklist*/).toString().split(",", QString::SkipEmptyParts);
            QMap<int, QString> ch_billno_map = restraurantDelegate::get_ch_billno_div_map(modeltable->data(index, Qt::UserRole + 2/*ch_billnos*/).toString());

            //分单满单， 未分单满单
            if(ch_billno_map.count() == 9
                    || (ch_billno_map.count() == 1 && !ch_billno_map.value(0).isEmpty())) {
                lds_messagebox::warning(this, MESSAGE_TITLE_VOID, ui->pushButton_destination->text() + tr("操作失败"));
                return;
            }
            if(restraurantDelegate::ch_billno_is_div(ch_billno_map) && check_to_list.isEmpty()) {//已分单、没有分单号，操作错误
                ...
                QString transferinfo =
                        ui->pushButton_cur_table->text()+":"+ui->label_cur_table->text()        +  ( _9Box->iscombine ? "" : ( "," + tr("分单号") + indexlist[0].data(Qt::UserRole + 1).toString() ))+"\n" +
                        ui->pushButton_destination->text()+":"+ui->lineEdit_destination->text() +  (check_to_list.isEmpty() ? "" : ( "," + tr("分单号") + check_to_list[0]))//都是单选
                        ;
                if(0 == lds_messagebox::information(this, MESSAGE_TITLE_VOID,transferinfo,tr("确认"), tr("取消"))){
                    ...
                    if(query.execUpdate("cey_u_table","ch_tableno", ch_tableno_to, qrtEqual("ch_billno", ch_billno_from))) {
                        if(query.execUpdate("cey_u_orderdish",qrtVariantPairList()
                            ...
                            if(query.execUpdate("cey_u_orderdish_print_tr","vch_plan_k3o3_state", state, qrtEqual("ch_billno", ch_billno_from))) {
                                if(w_scr_dish_restaurant_div_pop::cey_bt_table_update(ch_tableno_from, "", div_sqr_from)) {
                                    if(w_scr_dish_restaurant_div_pop::cey_bt_table_update(ch_tableno_to, ch_billno_from, div_sqr_to)) {
                                        lds_query::com_mit();
